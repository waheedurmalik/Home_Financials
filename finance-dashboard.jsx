  ...
  return React.createElement("div",{style:{paddingBottom:40}},
    ...
    React.createElement("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:8}},
      ...
      React.createElement("div",{style:{display:"flex",gap:8,alignItems:"center"}},
        React.createElement("button",{onClick:function(){setManageInitSection("financial");setManageInitFinancialSub(null);openModal("manage");},
...
function HomeTab({transactions, financials, budgets, taxonomy, displayCurrency, dispRates, globalTypeFilter, setTab, setModal, setManageInitSection, setManualInitMode, setManualFromHome, setPositionUnlocked, spikeThreshold, forecastProjection, isWide, drawerOpen, setDrawerOpen}) {
  ...
  return (
    <div style={{padding:isWide?"24px 28px":"16px 12px",maxWidth:1200,margin:"0 auto"}}>
      ...
      {(function(){
        var accts = ((financials&&financials.accounts)||[]).concat((financials&&financials.cash)||[]);
        ...
        return (
          <div style={{..._surf,padding:"18px 20px",marginTop:12}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,flexWrap:"wrap",gap:6}}>
              <div style={{fontSize:13,fontWeight:500,color:"#0f1624"}}>Financial Accounts Status</div>
              <span onClick={function(){navTo("position");}} style={{fontSize:12,color:"#2a9d6f",cursor:"pointer",fontWeight:500}}>Position ›</span>
            </div>
            {updStr&&<div style={{fontSize:11,color:"#7a8699",marginBottom:10}}>Last updated: {updStr}</div>}
            {!updStr&&<div style={{fontSize:11,color:"#f4a261",marginBottom:10}}>Not yet updated — tap Update position to record balances</div>}
            <div style={{display:"grid",gridTemplateColumns:isWide?"1fr 1fr 1fr":"1fr 1fr",gap:8}}>
              {accts.map(function(a,i){
                var bal = a.balance!==undefined ? a.balance : (a.currentValue!==undefined ? a.currentValue : null);
                var name = a.name+(a.last4?" ···"+a.last4:"");
                var typeLabel = a.type||"Account";
                return (
                  <div key={i} style={{background:"#f8f9fb",borderRadius:8,padding:"10px 12px",border:"1px solid #eef0f3"}}>
                    <div style={{fontSize:10,color:"#7a8699",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:3}}>{typeLabel}</div>
                    <div style={{fontSize:12,fontWeight:600,color:"#0f1624",marginBottom:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{name}</div>
                    <div style={{fontSize:14,fontWeight:700,color:"#2a9d6f",fontVariantNumeric:"tabular-nums"}}>{fmtBal(bal,a.currency)}</div>
                  </div>
                );
              })}
            </div>
  ...
  var ADD_SECTIONS = [
    ...
    },
    {
      ...
      items:[
        ...
        {label:"Update position", action:function(){ setManualInitMode("position"); setManualFromHome(false); openModal("manual"); setAddSheetOpen(false); }},
      ]
    },
  ];
    ...
    return (
      <div style={{width:w,flexShrink:0,background:C.surface,borderRight:"1.5px solid "+C.border,
        ...
        <div style={{padding:sidebarCollapsed?"12px 0":"14px 16px 10px",borderBottom:"1px solid "+C.border,
          ...
          <img src="Home_financials_LOGO_White_Back.png" alt="Home Financials" style={{width:44,height:44,borderRadius:8,objectFit:"contain",flexShrink:0}}/>
          {!sidebarCollapsed&&(
            <div>
        ...
        <div style={{position:"fixed",top:0,left:0,right:0,height:52,
          ...
          <div style={{display:"flex",alignItems:"center",gap:7,padding:"0 10px",flexShrink:0,borderRight:"0.5px solid "+C.border}}>
            <img src="Home_financials_LOGO_White_Back.png" alt="Home Financials"
                 style={{width:34,height:34,borderRadius:6,objectFit:"contain",flexShrink:0}}/>
            <div>
