...
function FinancialSection({financials, setFinancials, taxonomy}) {
  const [subSection, setSubSection] = useState("accounts");
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(13,15,14,0.96)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{position:"relative",background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:28,maxWidth:560,width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        {section==="vendors"&&(
          <div>
            {/* F1/F2 — Three-tab toggle */}
            <div style={{display:"flex",gap:2,background:C.s2,borderRadius:10,padding:3,marginBottom:12}}>
              {[["all","All Vendors"],["unassigned","⚠️ No Category"],["no_vendor","🏷️ No Vendor Yet"]].map(([v,l])=>(
                <button key={v} onClick={()=>{setVendorTab(v);setVendorSel(new Set());setVendorSaved(false);setNvAssign({});setNvSaved(null);}} style={{flex:1,padding:"7px 0",borderRadius:8,border:"none",background:vendorTab===v?C.surface:"transparent",color:vendorTab===v?C.text:C.muted,fontWeight:vendorTab===v?700:500,fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>{l}</button>
              ))}
            </div>

            {/* ── F2 — No Vendor Yet tab ── */}
            {vendorTab==="no_vendor"&&(()=>{
              // Group txs with no vendor by raw description
              const noVendorTxs=(rawTxs||[]).filter(t=>!t.vendor);
              const descMap={}; // description -> {count, total}
              noVendorTxs.forEach(t=>{
                const k=t.description||"(no description)";
                if(!descMap[k]) descMap[k]={count:0,total:0};
                descMap[k].count++;
                descMap[k].total+=t.isCredit?t.amount:-t.amount;
              });
              const descs=Object.keys(descMap).sort();
              if(!descs.length) return (
                <div style={{padding:"32px 14px",textAlign:"center",color:C.accent,fontSize:14,fontWeight:600}}>
                  ✓ All transactions have a vendor assigned!
                </div>
              );
              return (
                <div>
                  <div style={{fontSize:12,color:C.muted,marginBottom:10,lineHeight:1.5}}>
                    {noVendorTxs.length} transaction{noVendorTxs.length!==1?"s":""} across {descs.length} description{descs.length!==1?"s":""} have no vendor. Type a clean vendor name and tap Assign to bulk-set <strong style={{color:C.text}}>tx.vendor</strong> on all matching transactions.
                  </div>
                  <div style={{border:`1px solid ${C.border}`,borderRadius:10,background:C.bg,overflow:"hidden"}}>
                    {descs.map((desc,i)=>{
                      const {count,total}=descMap[desc];
                      const inputVal=nvAssign[desc]||"";
                      const saved=nvSaved===desc;
                      return (
                        <div key={desc} style={{borderBottom:i<descs.length-1?`1px solid ${C.s2}`:"none",padding:"10px 14px"}}>
                          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:6}}>
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{fontSize:12,fontWeight:600,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{desc}</div>
                              <div style={{fontSize:11,color:C.muted,marginTop:2}}>{count} tx · {total<0?"−":"+"}{Math.abs(total).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</div>
                            </div>
                            {saved&&<span style={{fontSize:11,color:C.accent,fontWeight:700,flexShrink:0}}>✓ Saved</span>}
                          </div>
                          <div style={{display:"flex",gap:6}}>
                            <input
                              value={inputVal}
                              onChange={e=>setNvAssign(prev=>({...prev,[desc]:e.target.value}))}
                              placeholder="e.g. Amazon, DEWA, Carrefour…"
                              style={{...inp({padding:"6px 10px",fontSize:12}),flex:1}}
                            />
                            <button
                              onClick={()=>{
                                const vName=(inputVal||"").trim();
                                if(!vName) return;
                                setRawTxs(prev=>prev.map(t=>t.description===desc&&!t.vendor?{...t,vendor:vName}:t));
                                setNvSaved(desc);
                                setNvAssign(prev=>({...prev,[desc]:""}));
                                setTimeout(()=>setNvSaved(null),2500);
                              }}
                              disabled={!inputVal.trim()}
                              style={{...btn(inputVal.trim()?C.accent:C.s3,inputVal.trim()?"#fff":C.dim,"none",12,"6px 12px"),flexShrink:0,opacity:inputVal.trim()?1:0.5}}
                            >Assign</button>
                          </div>
                        </div>
                      );
            ...
            {/* ── F1 — All Vendors / No Category tabs ── */}
            {vendorTab!=="no_vendor"&&(
              <div>
                <input
                ...
                />
                {/* F1 — Vendor list grouped by tx.vendor */}
                <div style={{maxHeight:260,overflowY:"auto",border:`1px solid ${C.border}`,borderRadius:10,background:C.bg,marginBottom:14}}>
                  {(()=>{
                    // Build vendor groups: key = tx.vendor (clean name)
                    const vMap={}; // vendorName -> {count, total, descriptions: Set}
                    (rawTxs||[]).forEach(t=>{
                      const key=t.vendor||(t.description||"(no description)");
                      if(!vMap[key]) vMap[key]={count:0,total:0,descriptions:new Set(),hasVendor:!!t.vendor};
                      vMap[key].count++;
                      vMap[key].total+=t.isCredit?t.amount:-t.amount;
                      vMap[key].descriptions.add(t.description||"");
                    });
                    // For category filtering: check all descriptions for this vendor against vendorMap
                    const vendorHasCat=(key)=>{
                      const g=vMap[key];
                      if(!g) return false;
                      for(const d of g.descriptions) if(vendorMap[d.toLowerCase().trim()]) return true;
                      return false;
                    };
                    let keys=Object.keys(vMap).sort((a,b)=>a.localeCompare(b));
                    if(vendorTab==="unassigned") keys=keys.filter(k=>!vendorHasCat(k));
                    if(vendorSearch) keys=keys.filter(k=>k.toLowerCase().includes(vendorSearch.toLowerCase()));
                    if(!keys.length) return (
                      <div style={{padding:"24px 14px",textAlign:"center",color:C.dim,fontSize:13}}>
                        {vendorTab==="unassigned"?"All vendors have categories assigned 🎉":"No vendors found"}
                      </div>
                    );
                    return keys.map(vendorKey=>{
                      const g=vMap[vendorKey];
                      const on=vendorSel.has(vendorKey);
                      // Get category from first matching description in vendorMap
                      let vm=null;
                      for(const d of g.descriptions){const k=d.toLowerCase().trim(); if(vendorMap[k]){vm=vendorMap[k];break;}}
                      return (
                        <div key={vendorKey} style={{borderBottom:`1px solid ${C.s2}`,background:on?"rgba(42,157,111,0.06)":"transparent"}}>
                          {/* F3 — Rename inline */}
                          {renamingVendor&&renamingVendor.old===vendorKey ? (
                            <div style={{padding:"8px 14px",display:"flex",gap:8,alignItems:"center"}} onClick={e=>e.stopPropagation()}>
                              <input
                                value={renamingVendor.newName}
                                onChange={e=>setRenamingVendor(prev=>({...prev,newName:e.target.value}))}
                                style={{...inp({padding:"5px 8px",fontSize:13}),flex:1}}
                                autoFocus
                              />
                              <button onClick={()=>{
                                const oldName=renamingVendor.old;
                                const newName=renamingVendor.newName.trim();
                                if(!newName||newName===oldName){setRenamingVendor(null);return;}
                                // F3: update tx.vendor on all transactions where tx.vendor===oldName
                                // vendorMap keys (raw descriptions) are NOT changed
                                setRawTxs(prev=>prev.map(t=>{
                                  if(t.vendor===oldName) return {...t,vendor:newName};
                                  // If no vendor and description was being used as key, update vendor field
                                  if(!t.vendor&&(t.description||"(no description)")===oldName) return {...t,vendor:newName};
                                  return t;
                                }));
                                setRenamingVendor(null);
                              }} style={btn(C.accent,"#fff","none",12,"4px 10px")}>✓</button>
                              <button onClick={()=>setRenamingVendor(null)} style={btn(C.s2,C.muted,`1px solid ${C.border}`,12,"4px 8px")}>✕</button>
                            </div>
                          ):(
                            <div onClick={()=>setVendorSel(prev=>{const s=new Set(prev);s.has(vendorKey)?s.delete(vendorKey):s.add(vendorKey);return s;})} style={{padding:"9px 14px",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
                              ...
                              <div style={{flex:1,minWidth:0}}>
                                <div style={{fontSize:13,fontWeight:600,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{vendorKey}</div>
                                <div style={{fontSize:11,color:C.muted,marginTop:1}}>{g.count} tx · {g.total<0?"−":"+"}{Math.abs(g.total).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}{vm&&<span style={{marginLeft:6,color:C.accent}}>{vm.category} › {vm.subcategory}</span>}</div>
                              </div>
                              {g.hasVendor&&<button onClick={e=>{e.stopPropagation();setRenamingVendor({old:vendorKey,newName:vendorKey});}} style={{...btn(C.s2,C.muted,`1px solid ${C.border}`,10,"2px 7px"),flexShrink:0}}>✏️</button>}
                            </div>
                          )}
                        </div>
                      );
                ...
                {/* F4 — Category assignment panel (unchanged logic, adapted for vendor groups) */}
                {vendorSel.size>0&&(
                  <div style={{background:C.s2,borderRadius:12,padding:"14px 16px",marginBottom:14}}>
                    <div style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:10}}>Assign category to {vendorSel.size} vendor{vendorSel.size!==1?"s":""}</div>
                    <div style={{marginBottom:10}}>
                      <div style={{fontSize:11,fontWeight:600,color:C.muted,marginBottom:5}}>Category</div>
                      <select value={vendorCat} onChange={e=>{setVendorCat(e.target.value);setVendorSub("");}} style={{...inp(),fontSize:13,appearance:"none"}}>
                    ...
                    {vendorCat&&(
                      <div style={{marginBottom:14}}>
                        <div style={{fontSize:11,fontWeight:600,color:C.muted,marginBottom:5}}>Subcategory</div>
                        <select value={vendorSub} onChange={e=>setVendorSub(e.target.value)} style={{...inp(),fontSize:13,appearance:"none"}}>
                          <option value="">— Select subcategory —</option>
                          {Object.keys(taxonomy[vendorCat]?.subs||{}).map(s=><option key={s} value={s}>{taxonomy[vendorCat]?.subs[s]?.icon} {s}</option>)}
                        </select>
                      </div>
                    )}
                    {vendorCat&&vendorSub&&(
                      <div style={{marginBottom:14}}>
                        <div style={{fontSize:11,fontWeight:600,color:C.muted,marginBottom:5}}>Cost Type</div>
                        <div style={{display:"flex",gap:6}}>
                          {Object.entries(TYPE_META).map(([v,m])=>(
                            <button key={v} onClick={()=>setVendorTxType(v)} style={{flex:1,padding:"8px 0",borderRadius:10,border:`1px solid ${vendorTxType===v?m.color:C.border}`,background:vendorTxType===v?m.color+"18":"transparent",color:vendorTxType===v?m.color:C.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",fontWeight:vendorTxType===v?700:500}}>{m.label}</button>
                          ))}
                        </div>
                      </div>
                    )}
                    {vendorSaved&&<div style={{color:C.accent,fontSize:13,fontWeight:600,marginBottom:10}}>✓ Saved — {vendorSel.size} vendor{vendorSel.size!==1?"s":""} updated, all transactions reassigned</div>}
                    <button
                      onClick={()=>{
                        if(!vendorCat||!vendorSub) return;
                        // F4: for each selected vendor key, find all raw descriptions that map to it
                        // and update vendorMap + stamp transactions
                        const newMap={...vendorMap};
                        const descSet=new Set();
                        // Build reverse map: vendorKey -> descriptions
                        const vDescMap={};
                        (rawTxs||[]).forEach(t=>{
                          const key=t.vendor||(t.description||"(no description)");
                          if(!vDescMap[key]) vDescMap[key]=new Set();
                          vDescMap[key].add(t.description||"");
                        });
                        vendorSel.forEach(vendorKey=>{
                          const descsForVendor=vDescMap[vendorKey]||new Set([vendorKey]);
                          descsForVendor.forEach(d=>{
                            newMap[d.toLowerCase().trim()]={category:vendorCat,subcategory:vendorSub,txType:vendorTxType||"standard"};
                            descSet.add(d);
                          });
                        });
                        setVendorMap(newMap);
                        if(setRawTxs){
                          setRawTxs(prev=>prev.map(t=>{
                            const key=t.vendor||(t.description||"(no description)");
                            if(vendorSel.has(key)) return {...t,category:vendorCat,subcategory:vendorSub,txType:vendorTxType||"standard",_manual:true};
                            return t;
                          }));
                        }
                        setVendorSaved(true);
                        setVendorSel(new Set());
                        setVendorCat(""); setVendorSub(""); setVendorTxType("standard");
                      }}
                      disabled={!vendorCat||!vendorSub}
                      style={{...btn(vendorCat&&vendorSub?C.accent:C.s3,vendorCat&&vendorSub?"#fff":C.dim,vendorCat&&vendorSub?"none":`1px solid ${C.border}`,14,"12px 0"),width:"100%",opacity:vendorCat&&vendorSub?1:0.5}}
                    >
                      ✓ Save & Update All Transactions
                    </button>
                  </div>
                )}
              </div>
        ...
        {section==="financial"&&(
          <FinancialSection financials={financials} setFinancials={setFinancials} taxonomy={taxonomy}/>
        )}
        ...
        <LabelRow label="Apply to">
          <div style={{display:"flex",gap:8}}>
            {[["vendor","All from this vendor"],["this","This transaction only"]].map(([v,l])=>(
              <button key={v} onClick={()=>setScope(v)} style={{flex:1,padding:"9px 0",borderRadius:10,border:`1px solid ${scope===v?C.accent:C.border}`,background:scope===v?"rgba(62,180,137,0.08)":"transparent",color:scope===v?C.accent:C.muted,fontSize:12,cursor:"pointer",fontFamily:"inherit",fontWeight:scope===v?600:400,textAlign:"center"}}>
                {l}
              </button>
            ))}
          </div>
        </LabelRow>
        {scope==="vendor"&&accountId&&(
          <div style={{fontSize:11,color:"#d4860a",marginBottom:8,padding:"6px 10px",background:"rgba(212,134,10,0.08)",borderRadius:8}}>
            ⚠️ Account change will apply to this transaction only, even when saving vendor-wide.
          </div>
        )}
        <div style={{display:"flex",gap:10,marginTop:4}}>
          <button onClick={()=>onSave(cat,sub,txType,scope,accountId||null,vendor.trim()||null)} style={{...btn(C.accent,"#fff","none",14,"12px 0"),flex:1}}>Save</button>
...
// ─── Bulk Edit Modal ──────────────────────────────────────────────────────────
function BulkEditModal({selected,transactions,taxonomy,accounts,onSave,onClose}) {
  const txs=transactions.filter(t=>selected.has(t.date+"||"+t.description+"||"+t.amount));
  const [cat,setCat]=useState("");
  const [sub,setSub]=useState("");
  const [txType,setTxType]=useState("");
  const [bulkAccountId,setBulkAccountId]=useState("_keep"); // "_keep" | "" (unassign) | accountId

  const hasChanges = cat||sub||txType||(bulkAccountId!=="_keep");
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(13,15,14,0.96)",zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:28,maxWidth:480,width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <button
            onClick={()=>onSave(cat,sub,txType,selected,bulkAccountId)}
            disabled={!hasChanges}
...
function SettingsModal({taxonomy,vendorMap,rawTxs,currency,onImport,onClose}) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(13,15,14,0.96)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20,overflowY:"auto"}}>
...
function HomeTab({transactions, financials, budgets, taxonomy, displayCurrency, dispRates, globalTypeFilter, setTab, setModal, setManageInitSection, setManualInitMode, setManualFromHome, setPositionUnlocked, spikeThreshold, forecastProjection}) {
  ...
  function Scene(props){
    ...
    return (
      <div
        onTouchStart={function(e){ _touchStartX=e.touches[0].clientX; _touchStartY=e.touches[0].clientY; }}
        ...
        onClick={function(e){ if(e.target.closest&&e.target.closest("button"))return; flipCard(props.idx); }}
  ...
  function CardFront(props){
    ...
    var cardH   = props.isWide?220:178;
    var imgSz   = Math.round(cardH*0.85);
    return (
      <div style={{position:"absolute",inset:0,borderRadius:22,
        ...
        {/* icon glyph — PNG at 85% card height if available, SVG fallback */}
        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",paddingBottom:showImg?10:glyphPb,paddingTop:22,filter:shadow,zIndex:1,overflow:"hidden"}}>
          <div style={{width:showImg?imgSz:glyphSz,height:showImg?imgSz:glyphSz,display:"flex",alignItems:"center",justifyContent:"center",fontSize:glyphSz,lineHeight:1}}>
            {showImg
              ? <img src={props.imgSrc} alt="" width={imgSz} height={imgSz} style={{objectFit:"contain",display:"block"}} onError={function(){setImgFailed(true);}}/>
              : props.glyph}
          </div>
        </div>
        {/* top-left label */}
        ...
        <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"9px 13px 12px",background:"linear-gradient(to top,rgba(0,0,0,0.52) 0%,transparent 100%)",zIndex:2}}>
          <div style={{fontSize:props.isWide?14:12.5,fontWeight:800,color:"#fff",letterSpacing:"-0.2px",lineHeight:1.15}}>{props.name}</div>
          <div style={{fontSize:props.isWide?10:9,color:"rgba(255,255,255,0.6)",fontWeight:500,marginTop:2,lineHeight:1.3}}>{props.desc}</div>
        </div>
      </div>
    );
  }
  ...
  function GoBtn({onClick,children}){
    var _tx=0,_ty=0;
    function onTS(e){ _tx=e.touches[0].clientX; _ty=e.touches[0].clientY; }
    function onTE(e){
    ...
    return <button onTouchStart={onTS} onTouchEnd={onTE} onClick={function(e){e.stopPropagation();onClick();}} style={{display:"block",width:"100%",marginTop:"auto",padding:"6px 0",background:"rgba(255,255,255,0.10)",border:"1px solid rgba(255,255,255,0.18)",borderRadius:10,color:"#fff",fontSize:10.5,fontWeight:700,textAlign:"center",cursor:"pointer",fontFamily:"inherit"}}>{children}</button>;
  }
  ...
  // ── RENDER ──
  return (
    <div style={{paddingBottom:32,maxWidth:isWide?1400:520,margin:"0 auto"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:isWide?"1fr 1fr 1fr 1fr":"1fr 1fr",gap:11,padding:"0 16px",marginBottom:14}}>
        ...
        {Scene({idx:1,height:isWide?220:178,children:[
          CardFront({isWide:isWide,imgSrc:"/Home_Financials/card1-icon.png",bg:"linear-gradient(140deg,#1a6a3a 0%,#2a9d6f 100%)",section:"Summary",
            glyph:<svg viewBox="0 0 100 90" width={isWide?116:82} height={isWide?116:82} xmlns="http://www.w3.org/2000/svg">
              ...
              {/* Entablature */}
              <rect x="8" y="22" width="84" height="9" rx="1" fill="rgba(255,255,255,0.85)"/>
              {/* BANK label */}
              <text x="50" y="29.5" textAnchor="middle" fontSize="6.5" fontWeight="800" fontFamily="Arial,sans-serif" fill="#1a6a3a" letterSpacing="1.5">BANK</text>
              {/* Columns */}
              {[17,32,50,68,83].map(function(x,i){ return <rect key={i} x={x-4} y="31" width="8" height="34" rx="3" fill="rgba(255,255,255,0.80)"/>; })}
              {/* Base */}
              <rect x="6" y="65" width="88" height="7" rx="2" fill="rgba(255,255,255,0.85)"/>
              {/* Steps */}
              <rect x="3" y="72" width="94" height="5" rx="1.5" fill="rgba(255,255,255,0.65)"/>
              <rect x="0" y="77" width="100" height="5" rx="1.5" fill="rgba(255,255,255,0.45)"/>
              {/* Door */}
              <rect x="43" y="46" width="14" height="19" rx="2" fill="rgba(255,255,255,0.25)" stroke="rgba(255,255,255,0.5)" strokeWidth="1"/>
              {/* £ sign on door */}
              <text x="50" y="59" textAnchor="middle" fontSize="9" fontWeight="800" fontFamily="Arial,sans-serif" fill="rgba(255,255,255,0.95)">£</text>
              {/* Windows */}
              <rect x="15" y="46" width="12" height="10" rx="2" fill="rgba(255,255,255,0.25)" stroke="rgba(255,255,255,0.4)" strokeWidth="0.8"/>
              <rect x="73" y="46" width="12" height="10" rx="2" fill="rgba(255,255,255,0.25)" stroke="rgba(255,255,255,0.4)" strokeWidth="0.8"/>
            </svg>,
        ...
        {Scene({idx:2,height:isWide?220:178,children:[
          CardFront({isWide:isWide,imgSrc:"/Home_Financials/card2-icon.png",bg:"linear-gradient(140deg,#1e3a5f 0%,#2a5298 100%)",section:"Forecast",glyph:<svg viewBox="0 0 100 80" width={isWide?116:82} height={isWide?116:82} xmlns="http://www.w3.org/2000/svg">
              ...
              {/* Grid lines */}
              {[20,40,60].map(function(y,i){ return <line key={i} x1="8" y1={y} x2="96" y2={y} stroke="rgba(255,255,255,0.12)" strokeWidth="1"/>; })}
              {/* Area fill */}
              <polygon points="8,70 25,58 42,50 60,34 78,20 96,8 96,72 8,72" fill="url(#chartfill)"/>
              {/* Line */}
              <polyline points="8,70 25,58 42,50 60,34 78,20 96,8" fill="none" stroke="rgba(255,255,255,0.95)" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"/>
              {/* Dots */}
              {[[8,70],[25,58],[42,50],[60,34],[78,20],[96,8]].map(function(p,i){
                return <circle key={i} cx={p[0]} cy={p[1]} r={i===5?5:3} fill="white" opacity={i===5?1:0.7}/>;
              })}
              {/* Arrow on last dot */}
              <polygon points="96,2 100,10 92,10" fill="rgba(255,255,255,0.9)"/>
              {/* Axis */}
              <line x1="8" y1="8" x2="8" y2="72" stroke="rgba(255,255,255,0.35)" strokeWidth="1.5"/>
              <line x1="8" y1="72" x2="96" y2="72" stroke="rgba(255,255,255,0.35)" strokeWidth="1.5"/>
            </svg>,name:"Forecast Cashflow",desc:"36-month projection · events · CC payments"}),
          CardBack({children:[
          ...
          CardFront({isWide:isWide,imgSrc:"/Home_Financials/card3-icon.png",bg:"linear-gradient(140deg,#7a1a5e 0%,#b02880 100%)",section:"Analysis",glyph:
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:isWide?6:4}}>
              ...
              <div style={{display:"flex",gap:isWide?10:7}}>
                <span style={{fontSize:isWide?44:32,lineHeight:1}}>🚗</span>
                <span style={{fontSize:isWide?44:32,lineHeight:1}}>🛒</span>
              </div>
            </div>,
            name:"Income & Spend Analysis",desc:"Categories · subcategories · trends"}),
          CardBack({children:[
            BackHd({children:"This month"}),
            TxSect({children:"Top spend categories"}),
            catSpend.slice(0,2).map(function(e,i){
              return BarRow({key:e[0], label:e[0], pct:Math.round(e[1]/maxCat*100), col:i===0?"#ff7070":"#f5c06a", val:fmtK(e[1])});
            }),
            TxSect({children:"Top income"}),
            incomeBycat.slice(0,1).map(function(e){ return BarRow({key:e[0], label:e[0], pct:100, col:"#5de0a8", val:fmtK(e[1])}); }),
            <div style={{display:"flex",flexDirection:"column",gap:5,marginTop:"auto",paddingTop:5}}>
        ...
        {Scene({idx:4,height:isWide?220:178,children:[
          CardFront({
            ...
            section:"Transactions",
            glyph:<svg viewBox="0 0 150 108" width={isWide?150:130} height={isWide?109:95} xmlns="http://www.w3.org/2000/svg">
              <defs>
                <linearGradient id="amexbody" x1="0" y1="0" x2="1" y2="1">
                  <stop offset="0%" stopColor="#007b9e"/>
                  <stop offset="50%" stopColor="#005f80"/>
                  <stop offset="100%" stopColor="#003d5c"/>
                </linearGradient>
                <linearGradient id="amexshine" x1="0" y1="0" x2="0.5" y2="1">
                  <stop offset="0%" stopColor="rgba(255,255,255,0.20)"/>
                  <stop offset="60%" stopColor="rgba(255,255,255,0.04)"/>
                  <stop offset="100%" stopColor="rgba(255,255,255,0)"/>
                </linearGradient>
                <linearGradient id="amexband" x1="0" y1="0" x2="1" y2="1">
                ...
                <linearGradient id="chipg" x1="0" y1="0" x2="1" y2="1">
                  ...
                  <stop offset="40%" stopColor="#c8980a"/>
                  <stop offset="70%" stopColor="#edcc44"/>
                  <stop offset="100%" stopColor="#a07008"/>
                </linearGradient>
                <linearGradient id="chippad" x1="0" y1="1" x2="1" y2="0">
                  <stop offset="0%" stopColor="#a07008"/>
                  <stop offset="100%" stopColor="#ffe898"/>
                </linearGradient>
                <linearGradient id="apbody" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#2e2e2e"/>
                  <stop offset="100%" stopColor="#111111"/>
                </linearGradient>
                <linearGradient id="apshine" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="rgba(255,255,255,0.16)"/>
                  <stop offset="100%" stopColor="rgba(255,255,255,0)"/>
                </linearGradient>
                <filter id="carddrop">
                  <feDropShadow dx="2" dy="4" stdDeviation="4" floodColor="rgba(0,0,0,0.7)"/>
                </filter>
                <filter id="paydrop">
                  <feDropShadow dx="1" dy="3" stdDeviation="3" floodColor="rgba(0,0,0,0.6)"/>
                </filter>
              </defs>

              {/* ── AMEX CARD FRONT — tilted ── */}
              <g transform="rotate(-7, 48, 54)" filter="url(#carddrop)">
                <rect x="2" y="6" width="92" height="60" rx="6" fill="url(#amexbody)"/>
                <rect x="2" y="6" width="92" height="60" rx="6" fill="url(#amexshine)"/>
                <rect x="2" y="6" width="92" height="60" rx="6" fill="url(#amexband)"/>
                {/* AMERICAN EXPRESS wordmark top-right */}
                <rect x="52" y="6" width="42" height="14" fill="rgba(0,0,0,0.18)"/>
                <text x="73" y="12" textAnchor="middle" fontSize="4" fontWeight="900" fontFamily="Arial,sans-serif" fill="rgba(255,255,255,0.85)" letterSpacing="0.6">AMERICAN</text>
                <text x="73" y="18" textAnchor="middle" fontSize="4" fontWeight="900" fontFamily="Arial,sans-serif" fill="rgba(255,255,255,0.85)" letterSpacing="0.6">EXPRESS</text>
                {/* CENTURION — Amex iconic helmet silhouette */}
                <g transform="translate(68,36)" opacity="0.78">
                  <path d="M18,2 C22,2 26,5 26,10 C26,14 24,17 22,19 L22,24 L10,24 L10,19 C8,17 6,14 6,10 C6,5 10,2 14,2 Z" fill="rgba(255,255,255,0.22)" stroke="rgba(255,255,255,0.4)" strokeWidth="0.6"/>
                  <path d="M14,2 C14,2 10,-4 6,-6 C10,-4 18,-4 22,2" fill="rgba(255,255,255,0.30)" stroke="rgba(255,255,255,0.35)" strokeWidth="0.5"/>
                  <ellipse cx="11" cy="11" rx="1.5" ry="1.5" fill="rgba(255,255,255,0.5)"/>
                  <path d="M13,13 L11,16 L15,16" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="0.7" strokeLinejoin="round"/>
                  <path d="M10,19 Q14,22 18,19" fill="none" stroke="rgba(255,255,255,0.35)" strokeWidth="0.7"/>
                </g>
                {/* EMV CHIP */}
                <rect x="8" y="22" width="22" height="17" rx="3" fill="url(#chipg)" stroke="rgba(170,120,0,0.5)" strokeWidth="0.7"/>
                <rect x="10" y="24" width="6" height="4.5" rx="1" fill="url(#chippad)" stroke="rgba(130,90,0,0.4)" strokeWidth="0.4"/>
                <rect x="18" y="24" width="6" height="4.5" rx="1" fill="url(#chippad)" stroke="rgba(130,90,0,0.4)" strokeWidth="0.4"/>
                <rect x="10" y="30" width="6" height="4.5" rx="1" fill="url(#chippad)" stroke="rgba(130,90,0,0.4)" strokeWidth="0.4"/>
                <rect x="18" y="30" width="6" height="4.5" rx="1" fill="url(#chippad)" stroke="rgba(130,90,0,0.4)" strokeWidth="0.4"/>
                <line x1="8" y1="30.5" x2="30" y2="30.5" stroke="rgba(140,95,0,0.38)" strokeWidth="0.5"/>
                <rect x="9" y="23" width="20" height="3" rx="1.5" fill="rgba(255,240,140,0.4)"/>
                {/* CONTACTLESS */}
                <path d="M34,29 Q37,32.5 34,36" fill="none" stroke="rgba(255,255,255,0.65)" strokeWidth="1.5" strokeLinecap="round"/>
                <path d="M37,26 Q42,32.5 37,39" fill="none" stroke="rgba(255,255,255,0.45)" strokeWidth="1.5" strokeLinecap="round"/>
                <path d="M40,23 Q47,32.5 40,42" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="1.5" strokeLinecap="round"/>
                <circle cx="32" cy="32.5" r="1.4" fill="rgba(255,255,255,0.8)"/>
                {/* AMEX 15-DIGIT NUMBER */}
                <text x="8" y="52" fontSize="5.8" fontWeight="700" fontFamily="'Courier New',monospace" fill="rgba(255,255,255,0.90)" letterSpacing="1.4">•••• •••••• •4291</text>
                {/* CID — 4-digit code above number, Amex-specific */}
                <text x="50" y="40" fontSize="4.2" fontWeight="600" fontFamily="'Courier New',monospace" fill="rgba(255,255,255,0.55)" letterSpacing="1">1234</text>
                <text x="50" y="35" fontSize="3" fontWeight="500" fontFamily="Arial,sans-serif" fill="rgba(255,255,255,0.35)" letterSpacing="0.4">CID</text>
                {/* CARDHOLDER */}
                <text x="8" y="62" fontSize="5" fontWeight="600" fontFamily="Arial,sans-serif" fill="rgba(255,255,255,0.60)" letterSpacing="0.8">W MALIK</text>
                {/* EXPIRY */}
                <text x="62" y="62" fontSize="3.8" fontWeight="500" fontFamily="'Courier New',monospace" fill="rgba(255,255,255,0.42)" letterSpacing="0.5">09/28</text>
                {/* CARD RIM */}
                <rect x="2" y="6" width="92" height="60" rx="6" fill="none" stroke="rgba(255,255,255,0.16)" strokeWidth="0.8"/>
              </g>

              {/* ── APPLE PAY PILL ── */}
              <g transform="translate(70,58)" filter="url(#paydrop)">
                <rect x="0" y="0" width="74" height="42" rx="9" fill="url(#apbody)"/>
                <rect x="0" y="0" width="74" height="21" rx="9" fill="url(#apshine)"/>
                <rect x="0" y="0" width="74" height="42" rx="9" fill="none" stroke="rgba(255,255,255,0.14)" strokeWidth="0.8"/>
                {/* Apple logo */}
                <g transform="translate(9,6) scale(0.62)">
                  <path d="M23,5 C21,2 18,1 15,1 C12,1 9,3 7,3 C5,3 2,1 0,2 C-3,3 -5,7 -5,11 C-5,16 -2,22 1,26 C3,29 6,32 9,32 C11,32 13,31 15,31 C17,31 19,32 21,32 C24,32 27,29 29,26 C32,22 35,16 35,11 C35,6 32,2 28,1 C26,0 24,2 23,5Z" fill="white" opacity="0.96"/>
                  <path d="M15,0 C15,-2 18,-5 22,-4" fill="none" stroke="white" strokeWidth="2.2" strokeLinecap="round" opacity="0.96"/>
                </g>
                {/* Pay text */}
                <text x="44" y="27" textAnchor="middle" fontSize="18" fontWeight="300" fontFamily="-apple-system,BlinkMacSystemFont,'Helvetica Neue',Arial,sans-serif" fill="white" opacity="0.96" letterSpacing="-0.5">Pay</text>
              </g>
            </svg>,
        ...
        {Scene({idx:5,height:isWide?220:178,children:[
          CardFront({isWide:isWide,imgSrc:"/Home_Financials/card5-icon.png",bg:"linear-gradient(140deg,#8a5200 0%,#c8860a 100%)",section:"Budget",glyph:<svg viewBox="0 0 100 100" width={isWide?116:82} height={isWide?116:82} xmlns="http://www.w3.org/2000/svg">
              ...
              {/* Middle ring */}
              <circle cx="50" cy="50" r="30" fill="rgba(255,255,255,0.12)" stroke="rgba(255,255,255,0.7)" strokeWidth="3"/>
              {/* Inner ring */}
              <circle cx="50" cy="50" r="16" fill="rgba(255,255,255,0.92)" stroke="rgba(255,255,255,0.9)" strokeWidth="2"/>
              {/* Tick */}
              <polyline points="38,50 46,60 64,38" fill="none" stroke="#c8860a" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>,name:"Budget Status",desc:"Monthly limits · category targets · tracking"}),
          CardBack({children:[
        ...
        {Scene({idx:6,height:isWide?220:178,children:[
          CardFront({isWide:isWide,imgSrc:"/Home_Financials/card6-icon.png",bg:"linear-gradient(140deg,#7a1a1a 0%,#c0392b 100%)",section:"Spend Alert",glyph:<svg viewBox="0 0 100 100" width={isWide?116:82} height={isWide?116:82} xmlns="http://www.w3.org/2000/svg">
              {/* Motion lines */}
              ...
              <path d="M82,30 Q88,42 82,54" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="2.5" strokeLinecap="round"/>
              <path d="M89,22 Q98,42 89,62" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="2" strokeLinecap="round"/>
              {/* Bell body */}
              <path d="M50,14 C34,14 24,26 24,44 L24,64 L76,64 L76,44 C76,26 66,14 50,14 Z" fill="rgba(255,255,255,0.92)"/>
              {/* Bell top stem */}
              <rect x="46" y="8" width="8" height="8" rx="4" fill="rgba(255,255,255,0.85)"/>
              {/* Bell bottom clapper */}
              <rect x="34" y="63" width="32" height="6" rx="3" fill="rgba(255,255,255,0.75)"/>
              <circle cx="50" cy="76" r="7" fill="rgba(255,255,255,0.75)"/>
              {/* Alert dot */}
              <circle cx="74" cy="22" r="11" fill="#ff4444"/>
              <text x="74" y="26.5" textAnchor="middle" fontSize="12" fontWeight="900" fontFamily="Arial,sans-serif" fill="white">!</text>
            </svg>,name:"Spend Alert",desc:"Spike detection · over-limit warnings"}),
          CardBack({children:[
        ...
        {Scene({idx:7,height:isWide?220:178,children:[
          CardFront({isWide:isWide,imgSrc:"/Home_Financials/card7-icon.png",bg:"linear-gradient(140deg,#1a3a5a 0%,#1e5f8a 100%)",section:"Vendors",glyph:<svg viewBox="0 0 100 90" width={isWide?116:82} height={isWide?116:82} xmlns="http://www.w3.org/2000/svg">
              ...
              {/* Awning */}
              <path d="M4,24 L96,24 L88,42 L12,42 Z" fill="rgba(255,255,255,0.95)"/>
              <line x1="4" y1="24" x2="96" y2="24" stroke="rgba(255,255,255,0.5)" strokeWidth="2"/>
              {/* Awning stripes */}
              {[20,35,50,65,80].map(function(x,i){ return <line key={i} x1={x} y1="24" x2={x-8} y2="42" stroke="rgba(30,95,138,0.3)" strokeWidth="3"/>; })}
              {/* Sign */}
              <rect x="20" y="42" width="60" height="12" rx="2" fill="rgba(30,95,138,0.15)" stroke="rgba(255,255,255,0.4)" strokeWidth="0.8"/>
              <text x="50" y="51.5" textAnchor="middle" fontSize="7" fontWeight="700" fontFamily="Arial,sans-serif" fill="rgba(30,95,138,0.8)" letterSpacing="1">VENDORS</text>
              {/* Door */}
              <rect x="40" y="60" width="20" height="26" rx="2" fill="rgba(30,95,138,0.25)" stroke="rgba(255,255,255,0.5)" strokeWidth="1"/>
              <circle cx="57" cy="73" r="1.5" fill="rgba(255,255,255,0.7)"/>
              {/* Windows */}
              <rect x="12" y="60" width="20" height="16" rx="2" fill="rgba(30,95,138,0.2)" stroke="rgba(255,255,255,0.4)" strokeWidth="0.8"/>
              <rect x="68" y="60" width="20" height="16" rx="2" fill="rgba(30,95,138,0.2)" stroke="rgba(255,255,255,0.4)" strokeWidth="0.8"/>
              {/* Window cross */}
              <line x1="22" y1="60" x2="22" y2="76" stroke="rgba(255,255,255,0.3)" strokeWidth="0.8"/>
              <line x1="12" y1="68" x2="32" y2="68" stroke="rgba(255,255,255,0.3)" strokeWidth="0.8"/>
              <line x1="78" y1="60" x2="78" y2="76" stroke="rgba(255,255,255,0.3)" strokeWidth="0.8"/>
              <line x1="68" y1="68" x2="88" y2="68" stroke="rgba(255,255,255,0.3)" strokeWidth="0.8"/>
              {/* Ground */}
              <rect x="0" y="86" width="100" height="4" rx="2" fill="rgba(255,255,255,0.3)"/>
            </svg>,name:"Vendor Spend Analysis",desc:"Top merchants · where money goes"}),
          CardBack({children:[
            BackHd({children:"Top spend vendors · last 12 months"}),
            top12Vendors.top.slice(0,3).map(function(e,i){
            ...
            StatRow({label:"Total vendors", val:top12Vendors.total, col:"rgba(255,255,255,0.5)"}),
            GoBtn({onClick:function(){navTo("vendors");}, children:"Go to Vendors →"})
          ]})
        ]})}

        {/* 8. FINANCIAL POSITION — tap to navigate directly, no flip */}
        {(function(){
          var _td={};
          function goPos(e){
            if(e.type==="touchend"){
              if(Math.abs(e.changedTouches[0].clientX-(_td.x||0))>10||Math.abs(e.changedTouches[0].clientY-(_td.y||0))>10)return;
              e.preventDefault();
            }
            setPositionUnlocked(false); navTo("position");
          }
          return (
            <div
              onTouchStart={function(e){_td={x:e.touches[0].clientX,y:e.touches[0].clientY};}}
              onTouchEnd={goPos}
              onClick={goPos}
              style={{height:isWide?220:178,borderRadius:22,cursor:"pointer",position:"relative",overflow:"hidden",
                background:"linear-gradient(140deg,#1976d2 0%,#64b5f6 100%)"}}>
              <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",paddingBottom:10,paddingTop:22,filter:isWide?"drop-shadow(0 6px 22px rgba(0,0,0,0.38))":"drop-shadow(0 4px 16px rgba(0,0,0,0.28))",zIndex:1}}>
                {(function(){
                  var cardH8=isWide?220:178;
                  var imgSz8=Math.round(cardH8*0.95);
                  var sz=isWide?116:82;
                  var [c8img,setC8img]=useState(true);
                  return <div style={{width:c8img?imgSz8:sz,height:c8img?imgSz8:sz,display:"flex",alignItems:"center",justifyContent:"center"}}>
                    {c8img
                      ? <img src="/Home_Financials/card8-icon.png" alt="" width={imgSz8} height={imgSz8} style={{objectFit:"contain",display:"block"}} onError={function(){setC8img(false);}}/>
              ...
              <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"9px 13px 12px",background:"linear-gradient(to top,rgba(0,0,0,0.52) 0%,transparent 100%)",zIndex:2}}>
                <div style={{fontSize:isWide?14:12.5,fontWeight:800,color:"#fff",letterSpacing:"-0.2px",lineHeight:1.15}}>Financial Position</div>
                <div style={{fontSize:isWide?10:9,color:"rgba(255,255,255,0.6)",fontWeight:500,marginTop:2}}>Tap to view →</div>
              </div>
            </div>
          );
        })()}
              ...
              <div style={{position:"relative",width:"100%",height:"100%",
                ...
                <div style={{position:"absolute",inset:0,borderRadius:16,backfaceVisibility:"hidden",WebkitBackfaceVisibility:"hidden",transform:"rotateY(180deg)",WebkitTransform:"rotateY(180deg)",background:"#1a1f2e",display:"flex",alignItems:"center",gap:7,padding:"0 12px"}}>
                  {[
                    ...
                    ["🔖","Tag\nAccounts", function(){ setFlippedAction(null); setManageInitSection("transactions"); setModal("manage"); }],
                  ].map(function(item,i){
                    return <button key={i} onClick={function(e){e.stopPropagation();item[2]();}}
                      style={{flex:1,height:46,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.11)",borderRadius:11,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,cursor:"pointer",fontFamily:"inherit",padding:"0 2px"}}>
                      <div style={{fontSize:17,lineHeight:1}}>{item[0]}</div>
...
function ScanDataPanel({onImport, onClose, taxonomy, vendorMap, financials}) {
  const [scanMode, setScanMode] = useState("pdf"); // "pdf" | "screenshot" | "spreadsheet"
  ...
  async function runScan() {
    ...
    try {
      ...
      if(scanMode==="pdf") {
        ...
        // Spreadsheet mode — rows parsed locally by XLSX, sent as tab-delimited text
        if(!xlsFile||!xlsRows||!xlsRows.length) { setErr("Select a spreadsheet first."); setStatus("idle"); return; }
        var rowText = xlsRows.slice(0,500).map(function(row){
          return row.map(function(cell){return cell===null||cell===undefined?"":String(cell);}).join(" | ");
        }).join("\n");
        var xlsResult = await callCloudFn({spreadsheetText: rowText});
        txs = xlsResult.transactions || xlsResult;
        if(xlsResult.piiReport) setPiiReport(xlsResult.piiReport);
        var xlsCost = calcGeminiCost(xlsResult.inputTokens||0, xlsResult.outputTokens||0);
        if(xlsCost>0||(xlsResult.inputTokens||0)>0) addLog("📊 Tokens: "+Math.round((xlsResult.inputTokens||0)/1000)+"K in / "+Math.round((xlsResult.outputTokens||0)/1000)+"K out · est. £"+xlsCost.toFixed(4));
        var xlsRec = {date:new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"}),mode:"spreadsheet",txCount:(txs||[]).length,inputTokens:xlsResult.inputTokens||0,outputTokens:xlsResult.outputTokens||0,costGBP:xlsCost};
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:8}}>
        {status!=="done"
          ? <button onClick={runScan} disabled={!hasInput||status==="loading"} style={{...btn(!hasInput||status==="loading"?C.s3:C.accent,!hasInput||status==="loading"?C.dim:"#fff","none",14,"12px 0"),flex:1,opacity:!hasInput||status==="loading"?0.5:1}}>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(13,15,14,0.96)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:16,overflowY:"auto"}}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:24,maxWidth:480,width:"100%",maxHeight:"94vh",overflowY:"auto"}}>
        ...
        {/* Description */}
        <div style={{marginBottom:14}}>
          <div style={{fontSize:11,color:C.dim,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:6}}>Description / Vendor</div>
        ...
        {/* ── SCAN MODE ── */}
        {mode==="scan"&&<ScanDataPanel onImport={onImport} onClose={onClose} taxonomy={taxonomy} vendorMap={vendorMap} financials={financials}/>}
...
function App() {
    ...
    saveTimerRef.current = setTimeout(async function() {
      ...
      try {
        ...
        var configSave = db.doc("users/"+uid).set({
          taxonomy, vendorMap, currency, budgets, spikeThreshold,
  ...
  function handleBulkSave(cat,sub,txType,selected,bulkAccountId) {
    setRawTxs(prev=>prev.map(t=>{
  ...
  const TAB_SVG={
    home: <svg viewBox="0 0 64 64" width="26" height="26" xmlns="http://www.w3.org/2000/svg"><polygon points="32,6 58,26 58,58 40,58 40,40 24,40 24,58 6,58 6,26" fill="#2a9d6f"/><rect x="24" y="40" width="16" height="18" rx="2" fill="#fff" opacity="0.9"/></svg>,
    summary: <svg viewBox="0 0 64 64" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><polygon points="32,4 56,14 8,14" fill="#1a7a44"/><rect x="8" y="14" width="48" height="6" rx="1" fill="#1a7a44" opacity="0.85"/><rect x="12" y="20" width="6" height="22" rx="2" fill="#1a7a44" opacity="0.7"/><rect x="22" y="20" width="6" height="22" rx="2" fill="#1a7a44" opacity="0.7"/><rect x="32" y="20" width="6" height="22" rx="2" fill="#1a7a44" opacity="0.7"/><rect x="42" y="20" width="6" height="22" rx="2" fill="#1a7a44" opacity="0.7"/><rect x="8" y="42" width="48" height="5" rx="1" fill="#1a7a44" opacity="0.85"/><rect x="6" y="47" width="52" height="4" rx="1" fill="#1a7a44" opacity="0.55"/></svg>,
    position: <svg viewBox="0 0 64 64" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><path d="M32,4 L56,14 L56,36 C56,50 46,58 32,62 C18,58 8,50 8,36 L8,14 Z" fill="#5a46c0" opacity="0.22" stroke="#5a46c0" strokeWidth="2.5"/><text x="32" y="42" textAnchor="middle" fontSize="22" fontWeight="900" fontFamily="Arial,sans-serif" fill="#5a46c0">£</text></svg>,
    forecast: <svg viewBox="0 0 64 64" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><line x1="6" y1="16" x2="6" y2="56" stroke="#2a5298" strokeWidth="2" opacity="0.45"/><line x1="6" y1="56" x2="58" y2="56" stroke="#2a5298" strokeWidth="2" opacity="0.45"/><polyline points="6,52 18,42 30,36 42,26 54,14" fill="none" stroke="#2a5298" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"/><polygon points="54,8 60,18 48,18" fill="#2a5298"/></svg>,
    budget: <svg viewBox="0 0 64 64" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><circle cx="32" cy="32" r="26" fill="none" stroke="#c8860a" strokeWidth="2.5" opacity="0.4"/><circle cx="32" cy="32" r="17" fill="none" stroke="#c8860a" strokeWidth="2.5" opacity="0.65"/><circle cx="32" cy="32" r="8" fill="#c8860a" opacity="0.2"/><polyline points="22,32 28,40 44,22" fill="none" stroke="#c8860a" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    watchout: <svg viewBox="0 0 64 64" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><path d="M32,10 C20,10 13,19 13,30 L13,46 L51,46 L51,30 C51,19 44,10 32,10 Z" fill="#0f8f68" opacity="0.85"/><rect x="18" y="45" width="28" height="5" rx="2.5" fill="#0f8f68" opacity="0.6"/><circle cx="32" cy="56" r="5" fill="#0f8f68" opacity="0.6"/><circle cx="50" cy="12" r="10" fill="#ff4444"/><text x="50" y="17" textAnchor="middle" fontSize="13" fontWeight="900" fontFamily="Arial,sans-serif" fill="white">!</text></svg>,
    categories: <svg viewBox="0 0 64 64" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><path d="M6,8 L40,8 L58,32 L40,56 L6,56 Z" fill="none" stroke="#b02880" strokeWidth="3" strokeLinejoin="round"/><circle cx="20" cy="30" r="6" fill="#b02880"/></svg>,
    subcategories: <svg viewBox="0 0 64 64" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><rect x="6" y="6" width="24" height="22" rx="4" fill="#b02880" opacity="0.85"/><rect x="34" y="6" width="24" height="22" rx="4" fill="#b02880" opacity="0.85"/><rect x="6" y="36" width="24" height="22" rx="4" fill="#b02880" opacity="0.42"/><rect x="34" y="36" width="24" height="22" rx="4" fill="#b02880" opacity="0.42"/><circle cx="18" cy="17" r="4" fill="white" opacity="0.75"/><circle cx="46" cy="17" r="4" fill="white" opacity="0.75"/></svg>,
    transactions: <svg viewBox="0 0 64 64" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><rect x="3" y="12" width="58" height="40" rx="7" fill="rgba(90,48,112,0.22)" stroke="#5a3070" strokeWidth="2"/><rect x="3" y="22" width="58" height="11" fill="#5a3070" opacity="0.55"/><rect x="9" y="37" width="18" height="11" rx="3" fill="#c89a1e" opacity="0.88"/><circle cx="46" cy="32" r="8" fill="rgba(235,30,30,0.68)"/><circle cx="56" cy="32" r="8" fill="rgba(255,160,0,0.68)"/></svg>,
    vendors: <svg viewBox="0 0 64 64" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><rect x="6" y="34" width="52" height="28" rx="3" fill="#1e5f8a" opacity="0.72"/><path d="M2,20 L62,20 L58,34 L6,34 Z" fill="#1e5f8a" opacity="0.92"/><line x1="18" y1="20" x2="14" y2="34" stroke="rgba(255,255,255,0.35)" strokeWidth="2.5"/><line x1="32" y1="20" x2="28" y2="34" stroke="rgba(255,255,255,0.35)" strokeWidth="2.5"/><line x1="46" y1="20" x2="42" y2="34" stroke="rgba(255,255,255,0.35)" strokeWidth="2.5"/><rect x="24" y="44" width="16" height="18" rx="2" fill="#1e5f8a" opacity="0.4" stroke="rgba(255,255,255,0.35)" strokeWidth="1"/></svg>,
  };
  const TABS=[["home","🏠"],["summary","sum"],["position","pos"],["forecast","fct"],["budget","bgt"],["watchout","wco"],["categories","cat"],["subcategories","sub"],["transactions","txn"],["vendors","ven"]];
  const TAB_LABELS={home:"Home",summary:"Summary",position:"Fin. Position",forecast:"Forecast",budget:"Budget",watchout:"Spend Alert",categories:"Categories",subcategories:"Subcategories",transactions:"Transactions",vendors:"Vendors"};
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"inherit",paddingBottom:90}}>
      ...
      <div style={{padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,background:"rgba(255,255,255,0.97)",backdropFilter:"blur(12px)",boxShadow:"0 1px 8px rgba(0,0,0,0.10)",zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",flexShrink:0}}>
          <img src="Home_financials_LOGO_White_Back.png" alt="Home Financials" onClick={()=>setTab("home")} style={{height:64,width:"auto",objectFit:"contain",cursor:"pointer"}}/>
        </div>
        <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
          {/* Sync status — compact */}
      ...
      <div style={{maxWidth:tab==="home"?undefined:1100,margin:"0 auto",padding:tab==="home"?"0":"20px 16px"}}>
        ...
        {/* Tabs — first element, visible only on transactions/categories/subcategories */}
        {tab!=="home"&&(
          ...
          <div style={{display:"flex",gap:2,padding:5,minWidth:"max-content"}}>
            {TABS.map(([t])=>(
              <button key={t} onClick={()=>{if(tab==="position"&&t!=="position")setPositionUnlocked(false);setTab(t);if(t!=="transactions")setTxViewMode("all");}} style={{minWidth:80,padding:"11px 12px",borderRadius:10,border:"none",background:tab===t?C.accent:"transparent",color:tab===t?"#fff":C.muted,cursor:"pointer",fontFamily:"inherit",fontWeight:tab===t?700:500,outline:"none",textAlign:"center",lineHeight:1.3,transition:"all 0.15s"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:26}}>{TAB_SVG[t]||"●"}</div>
                <div style={{fontSize:11,marginTop:3,letterSpacing:"0.01em",whiteSpace:"nowrap"}}>{TAB_LABELS[t]}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{marginBottom:16,display:(tab==="home"||tab==="summary"||tab==="position"||tab==="forecast"||tab==="budget"||tab==="watchout"||tab==="vendors")?"none":"block"}}>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginBottom:period.preset==="custom"?10:6}}>
            ...
            {/* Month selector — always visible, all months from all transactions */}
            {(()=>{
        ...
        {importMsg&&(
          <div style={{
            ...
            background:importMsg.type==="warn"?"rgba(212,134,10,0.1)":importMsg.type==="ok"?"rgba(42,157,111,0.1)":"rgba(90,111,214,0.1)",
            border:`1px solid ${importMsg.type==="warn"?"#d4860a":importMsg.type==="ok"?C.accent:"#5a6fd6"}`,
        ...
        <div style={{display:(tab==="home"||tab==="summary"||tab==="position"||tab==="forecast"||tab==="budget"||tab==="watchout"||tab==="vendors")?"none":"flex",gap:6,alignItems:"center",marginBottom:10}}>
          ...
          <button onClick={()=>setAvgMode(true)} style={{padding:"5px 14px",borderRadius:20,border:`1px solid ${avgMode?C.accent:C.border}`,background:avgMode?C.accent:"transparent",color:avgMode?"#fff":C.muted,fontSize:12,fontWeight:avgMode?700:500,cursor:"pointer",fontFamily:"inherit"}}>
            Avg / Month
          </button>
        ...
        <div style={{display:(tab==="home"||tab==="summary"||tab==="position"||tab==="forecast"||tab==="budget"||tab==="watchout"||tab==="vendors")?"none":"flex",gap:6,marginBottom:10,alignItems:"center"}}>
        ...
        {/* Cost type filter */}
        <div style={{display:(tab==="home"||tab==="summary"||tab==="position"||tab==="forecast"||tab==="budget"||tab==="watchout"||tab==="vendors")?"none":"flex",gap:6,flexWrap:"wrap",marginBottom:12,alignItems:"center"}}>
          {Object.entries(TYPE_META).map(([v,m])=>(
          ...
          {globalTypeFilter.length<3&&(
            <button onClick={()=>setGlobalTypeFilter(["standard","nonstandard","exceptional"])} style={{padding:"5px 13px",borderRadius:20,border:`1px solid ${C.border}`,background:"transparent",color:C.accent,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
              Show All
            </button>
          )}
        </div>
        ...
        <div style={{display:(tab==="home"||tab==="summary"||tab==="position"||tab==="forecast"||tab==="budget"||tab==="watchout"||tab==="vendors")?"none":"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12,marginBottom:18}}>
          {[
        ...
        {tab==="watchout" && (
          <WatchOutTab
            ...
            showSpikeSettings={showSpikeSettings}
            setShowSpikeSettings={setShowSpikeSettings}
            displayCurrency={displayCurrency}
        ...
        {tab==="categories" && (
          <Drill txs={mTxs} allTxs={transactions} cur={displayCurrency} dispRates={dispRates} taxonomy={taxonomy}
            onRemap={tx=>{setRemapTx(tx);setModal("remap");}}
            initCat={drillCat} initSub={drillSub}
            onClearInit={()=>{setDrillCat(null);setDrillSub(null);}}
            avgMode={avgMode} mMonths={mMonths}
          />
        )}

        {tab==="subcategories" && (
          <AllSubs transactions={mTxs} allTxs={transactions} taxonomy={taxonomy} currency={displayCurrency} dispRates={dispRates}
        ...
        {tab==="transactions" && (
          <div style={card}>
            ...
            <input
              ...
              style={{...inp(),width:"100%",marginBottom:10,boxSizing:"border-box",fontSize:13}}
            />
            {/* Account filter — inclusive additive multi-select, same model as category filter */}
            {(()=>{
              ...
              function toggleAll() {
                setTxAccFilter(allSelected ? new Set() : null);
              }
              function toggleAcc(id) {
              ...
              return React.createElement("div", {style:{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap",alignItems:"center"}},
                ...
                // All pill
                React.createElement("button", {key:"all", onClick:toggleAll,
                  style:{padding:"4px 10px",borderRadius:20,fontSize:11,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",
            ...
            <div style={{display:"flex",gap:8,marginBottom:8,alignItems:"center",flexWrap:"wrap"}}>
              {/* Type filter removed — use global Fixed/Variable/Exceptional pills and Costs/Credits/All above */}
              <Pill
                options={[
                  ["amount_desc",  isActive=>txSort==="amount_asc"?"Value ↑":"Value ↓"],
                  ["subcategory_asc", isActive=>txSort==="subcategory_desc"?"Z–A Sub":"A–Z Sub"],
                  ["description_asc", isActive=>txSort==="description_desc"?"Z–A Desc":"A–Z Desc"],
                  ["date_desc",    isActive=>txSort==="date_asc"?"Oldest":"Date ↓"]
                ]}
                value={txSort==="amount_asc"?"amount_desc":txSort==="subcategory_desc"?"subcategory_asc":txSort==="description_desc"?"description_asc":txSort==="date_asc"?"date_desc":txSort}
                onChange={v=>{
                  if(v==="amount_desc")      setTxSort(txSort==="amount_desc"?"amount_asc":"amount_desc");
                  else if(v==="subcategory_asc") setTxSort(txSort==="subcategory_asc"?"subcategory_desc":"subcategory_asc");
                  else if(v==="description_asc") setTxSort(txSort==="description_asc"?"description_desc":"description_asc");
                  else if(v==="date_desc")   setTxSort(txSort==="date_desc"?"date_asc":"date_desc");
                }}/>
              <span style={{marginLeft:"auto",fontFamily:"monospace",fontSize:12,color:C.dim}}>{filteredTxs.length} · {fmt(filteredTxs.reduce((s,t)=>s+t.amount,0),currency,dispRates)}</span>
            </div>
            {/* Category filter — inclusive additive multi-select.
                null = all selected (show everything).
                Empty Set = nothing selected (show only uncategorised).
                Set with items = show only those categories. */}
            {(function(){
              var allCats = Object.keys(taxonomy);
              var allSelected = txCatFilter===null;
              var noneSelected = txCatFilter!==null && txCatFilter.size===0;

              function toggleAll() {
                setTxCatFilter(allSelected ? new Set() : null);
                setTxSubFilter("all");
              }
              function toggleCat(cat) {
                setTxCatFilter(function(prev) {
                  var next = prev===null ? new Set(allCats) : new Set(prev);
                  if(next.has(cat)) next.delete(cat); else next.add(cat);
                  if(next.size===allCats.length) return null;
                  return next;
                });
                setTxSubFilter("all");
              }

              return React.createElement("div", {style:{display:"flex",gap:6,marginBottom:6,flexWrap:"wrap",alignItems:"center"}},
                React.createElement("span", {style:{fontSize:11,fontWeight:600,color:C.muted,flexShrink:0}}, "Category:"),
                // All pill — green when active, plain grey outline when deselected (no name change)
                React.createElement("button", {
                  key:"all",
                  onClick:toggleAll,
                  style:{padding:"4px 10px",borderRadius:20,fontSize:11,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",
                         ...
                         color:allSelected?"#fff":C.muted}},
                  "All"),
                // Category pills — green fill when active, grey fill with readable dark text when inactive
                ...allCats.map(function(cat){
                  var active = allSelected || (txCatFilter!==null && txCatFilter.has(cat));
                  var icon = (taxonomy[cat]&&taxonomy[cat].icon)||"";
            ...
            {/* Subcategory filter — only show when exactly one category is visible */}
            {(function(){
              var allCats = Object.keys(taxonomy);
              var includedCats = txCatFilter===null ? allCats : (txCatFilter ? [...txCatFilter] : []);
              if(includedCats.length!==1) return null;
              var solocat = includedCats[0];
              if(!taxonomy[solocat]) return null;
              return React.createElement("div", {style:{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap",alignItems:"center"}},
                React.createElement("span", {style:{fontSize:11,fontWeight:600,color:C.muted,flexShrink:0}}, "Sub:"),
                [["all","All"],...Object.keys(taxonomy[solocat]?.subs||{}).map(function(s){ return [s,(taxonomy[solocat]?.subs[s]?.icon||"")+" "+s]; })].map(function([v,l]){
                  return React.createElement("button", {key:v, onClick:function(){setTxSubFilter(v);},
                    style:{padding:"4px 10px",borderRadius:20,border:"1px solid "+(txSubFilter===v?C.accent:C.border),background:txSubFilter===v?C.accent:"transparent",color:txSubFilter===v?"#fff":C.muted,fontSize:11,fontWeight:txSubFilter===v?700:500,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}},
                    l);
                }));
            })()}
            <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center",flexWrap:"wrap"}}>
              ...
              <button onClick={()=>{
                // Find duplicates: same date + normalised description + amount + isCredit
                const seen=new Map(); // normKey -> index in mTxs (first occurrence)
                const dupeRawKeys=new Set(); // raw keys of duplicates only (NOT originals)
                const allGroupRawKeys=new Set(); // raw keys of originals + dupes (for display)
                mTxs.forEach((t,idx)=>{
                  ...
                  if(seen.has(k)){
                    // This is a duplicate — select it for deletion
                    dupeRawKeys.add(rawKey);
                    // Also add the original to the display group (but not to selected)
                    allGroupRawKeys.add(seen.get(k));
                    allGroupRawKeys.add(rawKey);
                  } else {
                    seen.set(k, rawKey);
                  }
                });
                if(dupeRawKeys.size===0){ setImportMsg({text:"No duplicates found ✓",type:"ok"}); return; }
                // Select ONLY the duplicates (not originals) — safe to delete immediately
                ...
                setDupesMode(true);
                // Store all group keys for display filtering
              ...
              {!filteredTxs.length?<div style={{padding:28,textAlign:"center",color:C.dim,fontSize:13}}>No transactions{txSearch?" matching your search":""}</div>:filteredTxs.map((tx,i)=>{
                ...
                return (
                  <div key={i} style={{padding:"10px 12px",borderBottom:`1px solid ${C.s2}`,background:isEditing?"rgba(42,157,111,0.05)":isSel?"rgba(62,180,137,0.06)":isCredit?"rgba(62,180,137,0.03)":"transparent"}}>
                    {/* Top row: checkbox + icon + description + amount */}
                    <div style={{display:"flex",alignItems:"center",gap:8}} onClick={()=>{if(!isEditing)toggleSelTx(tx);}}>
                      <div style={{width:18,height:18,borderRadius:4,border:`2px solid ${isSel?C.accent:C.border}`,background:isSel?C.accent:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:10,color:"#fff",fontWeight:700}}>
                        {isSel?"✓":""}
                      </div>
                      ...
                      {isEditing?(
                        <input
                          ...
                          style={{...inp({padding:"4px 8px",fontSize:13}),flex:1,fontWeight:600}}
                          autoFocus
                        />
                      ):(
                        <div style={{flex:1,minWidth:0}}>
                      ...
                      {isEditing?(
                        <input
                          ...
                          onChange={e=>setEditingTx(prev=>({...prev,amount:e.target.value}))}
                          onClick={e=>e.stopPropagation()}
                          type="number"
                          step="0.01"
                          style={{...inp({padding:"4px 8px",fontSize:13}),width:90,fontFamily:"monospace",textAlign:"right"}}
                        />
                      ):(
                        <div style={{fontFamily:"monospace",fontSize:13,fontWeight:600,color:isCredit?C.accent:C.danger,flexShrink:0}}>
                          {isCredit?"+":"-"}{fmtExact(tx.amount,currency)}
                        </div>
                      )}
                    </div>
                    {/* Bottom row: date + category + action buttons */}
                            ...
                            <button onClick={()=>{
                              ...
                              setRawTxs(prev=>prev.map(t=>
                                t.date===tx.date&&t.description===tx.description&&t.amount===tx.amount
                                  ? {...t, description:newDesc, amount:newAmt, _manual:true}
                                  : t
                              ));
                              setEditingTx(null);
