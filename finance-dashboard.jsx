...
function HomeTab({transactions, financials, budgets, taxonomy, displayCurrency, dispRates, globalTypeFilter, setTab, setModal, setManageInitSection, setManualInitMode, setManualFromHome, setPositionUnlocked, spikeThreshold, forecastProjection, isWide, drawerOpen, setDrawerOpen}) {
  ...
  function flipCard(n){ setModal(null); setFlippedCard(flippedCard===n?null:n); setFlippedAction(null); }
  function flipAction(k){ setModal(null); setFlippedAction(flippedAction===k?null:k); setFlippedCard(null); }
  ...
  return (
    <div style={{paddingBottom:32,maxWidth:isWide?1400:520,margin:"0 auto"}}>
      ...
      {!isWide&&(
        <div style={{padding:"0 16px",marginBottom:14}}>
          <button
            ...
            style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"14px 18px",borderRadius:16,border:"1px solid "+C.border,background:C.surface,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
            <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0}}>
              <div style={{width:22,height:2.5,borderRadius:2,background:C.accent}}/>
              <div style={{width:16,height:2.5,borderRadius:2,background:C.accent}}/>
              <div style={{width:22,height:2.5,borderRadius:2,background:C.accent}}/>
            </div>
            <div style={{flex:1,textAlign:"left"}}>
              <div style={{fontSize:13,fontWeight:700,color:C.text}}>Menu</div>
              <div style={{fontSize:10,color:C.dim,marginTop:1}}>Input · Manage · Tools</div>
            </div>
      ...
      {/* ── 8-card grid ── */}
      <div style={{display:"grid",gridTemplateColumns:isWide?"1fr 1fr 1fr":"1fr 1fr",gap:11,padding:"0 16px",marginBottom:14}}>
        ...
        {Scene({idx:4,height:isWide?220:178,children:[
          ...
          CardBack({children:[
            <div style={{display:"flex",flex:1,gap:0,flexDirection:"column",height:"100%"}}>
              <div style={{fontSize:8,color:"rgba(255,255,255,0.30)",letterSpacing:"0.06em",marginBottom:4,textTransform:"uppercase"}}>{monNames[now.getMonth()]} {now.getFullYear()}</div>
              <div style={{display:"flex",gap:0,flex:1}}>
                <div style={{flex:1,display:"flex",flexDirection:"column"}}>
                  ...
                  {topSpend.map(function(t,i){ return <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",padding:"2.5px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}><span style={{fontSize:9,color:"rgba(255,255,255,0.5)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"58%"}}>{(t.vendor||t.description||"").slice(0,15)}</span><span style={{fontFamily:"monospace",fontSize:9.5,fontWeight:700,color:"#ff7070"}}>{fmtK(t.amount)}</span></div>; })}
                  {topSpend.length===0&&<div style={{fontSize:9,color:"rgba(255,255,255,0.3)"}}>No data this month</div>}
                </div>
                ...
                <div style={{flex:1,display:"flex",flexDirection:"column"}}>
                  ...
                  {topIncome.map(function(t,i){ return <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",padding:"2.5px 0",borderBottom:"1px solid rgba(255,255,255,0.05)"}}><span style={{fontSize:9,color:"rgba(255,255,255,0.5)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"58%"}}>{(t.vendor||t.description||"").slice(0,15)}</span><span style={{fontFamily:"monospace",fontSize:9.5,fontWeight:700,color:"#5de0a8"}}>{fmtK(t.amount)}</span></div>; })}
                  {topIncome.length===0&&<div style={{fontSize:9,color:"rgba(255,255,255,0.3)"}}>No data this month</div>}
                </div>
              </div>
...
function App() {
  ...
  // ── Nav structure (sidebar + drawer) ─────────────────────────────────────
  var NAV_GROUPS = [
    { key:"input", icon:"📥", label:"Input data", items:[
      { icon:"📸", label:"PDF statement",      scanMode:"pdf",          action:function(){ if(modal==="manual"&&manualInitScanMode==="pdf"){ setModal(null);setManualFromHome(false); }else{ setManualInitMode("scan"); setManualInitScanMode("pdf"); setManualFromHome(true); openModal("manual"); } }},
      { icon:"🖼️", label:"Screenshot data",    scanMode:"screenshot",   action:function(){ if(modal==="manual"&&manualInitScanMode==="screenshot"){ setModal(null);setManualFromHome(false); }else{ setManualInitMode("scan"); setManualInitScanMode("screenshot"); setManualFromHome(true); openModal("manual"); } }},
      { icon:"📊", label:"XLS spreadsheet",    scanMode:"spreadsheet",  action:function(){ if(modal==="manual"&&manualInitScanMode==="spreadsheet"){ setModal(null);setManualFromHome(false); }else{ setManualInitMode("scan"); setManualInitScanMode("spreadsheet"); setManualFromHome(true); openModal("manual"); } }},
      { icon:"➕", label:"Add transaction",    action:function(){ if(modal==="manual"&&manualInitMode==="transaction"){ setModal(null);setManualFromHome(false); }else{ setManualInitMode("transaction"); setManualFromHome(true); openModal("manual"); } }},
      { icon:"💼", label:"Update position",    action:function(){ if(modal==="manual"&&manualInitMode==="position"){ setModal(null);setManualFromHome(false); }else{ setManualInitMode("position"); setManualFromHome(true); openModal("manual"); } }},
    ]},
    { key:"manage", icon:"🗂️", label:"Manage data", items:[
      { icon:"🔖", label:"Tag accounts",       action:function(){ if(modal==="manage"&&manageInitSection==="transactions"){ setModal(null); }else{ setManageInitSection("transactions"); setManageInitFinancialSub(null); openModal("manage"); } }},
      { icon:"🏷️", label:"Categories",         action:function(){ if(modal==="manage"&&manageInitSection==="categories"){ setModal(null); }else{ setManageInitSection("categories"); setManageInitFinancialSub(null); openModal("manage"); } }},
      { icon:"🏪", label:"Vendors",            action:function(){ if(modal==="manage"&&manageInitSection==="vendors"){ setModal(null); }else{ setManageInitSection("vendors"); setManageInitFinancialSub(null); openModal("manage"); } }},
      { icon:"🏦", label:"Accounts",           action:function(){ if(modal==="manage"&&manageInitFinancialSub==="accounts"){ setModal(null);setManageInitFinancialSub(null); }else{ setManageInitSection("financial"); setManageInitFinancialSub("accounts"); openModal("manage"); } }},
      { icon:"📈", label:"Investments",        action:function(){ if(modal==="manage"&&manageInitFinancialSub==="investments"){ setModal(null);setManageInitFinancialSub(null); }else{ setManageInitSection("financial"); setManageInitFinancialSub("investments"); openModal("manage"); } }},
      { icon:"🏠", label:"Properties",         action:function(){ if(modal==="manage"&&manageInitFinancialSub==="properties"){ setModal(null);setManageInitFinancialSub(null); }else{ setManageInitSection("financial"); setManageInitFinancialSub("properties"); openModal("manage"); } }},
      { icon:"💳", label:"Loans",              action:function(){ if(modal==="manage"&&manageInitFinancialSub==="loans"){ setModal(null);setManageInitFinancialSub(null); }else{ setManageInitSection("financial"); setManageInitFinancialSub("loans"); openModal("manage"); } }},
      { icon:"📊", label:"Forecast spend rows",action:function(){ if(modal==="manage"&&manageInitSection==="forecast"){ setModal(null); }else{ setManageInitSection("forecast"); setManageInitFinancialSub(null); openModal("manage"); } }},
    ]},
    { key:"tools", icon:"🔧", label:"Tools", items:[
      { icon:"💾", label:"Backup / restore",   action:function(){ if(modal==="settings"){ setModal(null); }else{ openModal("settings"); } }},
      { icon:"📤", label:"Export Excel",       action:function(){ if(modal==="export"){ setModal(null); }else{ openModal("export"); } }},
      { icon:"🤖", label:"Gemini usage",       action:function(){ if(modal==="manage"&&manageInitSection==="gemini"){ setModal(null); }else{ setManageInitSection("gemini"); setManageInitFinancialSub(null); openModal("manage"); } }},
    ]},
  ];

  function renderNav() {
    var panelStyle = isWide ? {
      width:210, flexShrink:0, background:C.surface,
      borderRight:"1.5px solid "+C.border,
      display:"flex", flexDirection:"column",
      overflowY:"auto", padding:"12px 0 24px",
    } : {
      position:"absolute", top:0, left:0, bottom:0, width:"82%", maxWidth:320,
      background:C.surface, borderRight:"1.5px solid "+C.border,
      display:"flex", flexDirection:"column",
      overflowY:"auto", padding:"12px 0 24px",
      zIndex:210,
      transform: drawerOpen ? "translateX(0)" : "translateX(-100%)",
      transition:"transform 0.28s cubic-bezier(0.32,0,0.18,1)",
      WebkitTransition:"-webkit-transform 0.28s cubic-bezier(0.32,0,0.18,1)",
    };
    ...
    var panel = (
      <div style={panelStyle}>
        <div style={{padding:"6px 16px 10px",borderBottom:"1px solid "+C.border,marginBottom:8}}>
          <div style={{fontSize:9,color:C.dim,fontWeight:600,letterSpacing:"0.06em"}}>hf-v96</div>
        </div>
        {NAV_GROUPS.map(function(group,gi){
          var collapsed = navCollapsed[group.key];
          return (
            <div key={gi} style={{marginBottom:4}}>
              <button
                onClick={function(){ setNavCollapsed(function(prev){ return Object.assign({},prev,{[group.key]:!prev[group.key]}); }); }}
                style={{display:"flex",alignItems:"center",width:"100%",padding:"8px 16px 4px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
                <span style={{fontSize:11,marginRight:5}}>{group.icon}</span>
                <span style={{fontSize:9,fontWeight:800,color:C.dim,letterSpacing:"0.10em",textTransform:"uppercase",flex:1}}>{group.label}</span>
                <span style={{fontSize:10,color:C.dim,transition:"transform 0.2s",display:"inline-block",transform:collapsed?"rotate(-90deg)":"rotate(0deg)"}}>▾</span>
              </button>
              {!collapsed && group.items.map(function(item,ii){
                return (
                  <button key={ii} onClick={function(){ doAction(item.action); }}
                    style={{display:"flex",alignItems:"center",gap:9,width:"100%",padding:"9px 16px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"inherit",textAlign:"left",borderRadius:0}}>
    ...
    return (
      <div style={{position:"absolute",top:0,left:0,right:0,bottom:0,zIndex:200,pointerEvents:drawerOpen?"auto":"none"}}>
        <div onClick={function(){ setDrawerOpen(false); }}
          style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.45)",
            opacity:drawerOpen?1:0,transition:"opacity 0.28s",WebkitTransition:"opacity 0.28s"}}/>
        {panel}
      </div>
    );
  }
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"inherit"}}>
      ...
      {/* manual + manage: rendered inside home flex container when tab=home (see above). For other tabs, render here with centred 30% backdrop */}
      {modal==="manual" && tab!=="home" && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.30)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      ...
      {modal==="export"   && tab!=="home" && <ExportModal transactions={transactions} taxonomy={taxonomy} currency={currency} onClose={()=>setModal(null)}/>}
      {modal==="settings" && tab!=="home" && <SettingsModal taxonomy={taxonomy} vendorMap={vendorMap} rawTxs={rawTxs} currency={currency} onImport={handleSettingsImport} onClose={()=>setModal(null)}/>}
      ...
      {tab==="home" ? (
        /* ── Home tab: sidebar (desktop) or drawer (mobile) + content ── */
        <div style={{display:"flex",minHeight:"calc(100vh - 82px)",position:"relative"}}>
          {renderNav()}
          <div style={{flex:1,minWidth:0}}>
            <HomeTab
          ...
          {/* ── Home modals: position:absolute on desktop (no backdrop), fixed+30% on mobile ── */}
          {modal==="manual" && (function(){
            if(isWide) return (
              <div key={modalKey} style={{position:"absolute",top:0,left:210,bottom:0,width:380,background:C.surface,borderRight:"1px solid "+C.border,overflowY:"auto",zIndex:400,boxShadow:"4px 0 24px rgba(0,0,0,0.10)",animation:"slideInLeft 0.28s cubic-bezier(0.32,0,0.18,1)"}}>
                <ManualEntryModal onImport={handleImport} taxonomy={taxonomy} vendorMap={vendorMap} vendorList={vendorList} financials={financials} setFinancials={setFinancials} onClose={()=>{setModal(null);setManualFromHome(false);}} initialMode={manualInitMode} initialScanMode={manualInitScanMode} singleSection={true} noOverlay={true}/>
              </div>
            );
            return (
          ...
          {modal==="export" && (function(){
            if(isWide) return (
              <div key={modalKey} style={{position:"absolute",top:0,left:210,bottom:0,width:380,background:C.surface,borderRight:"1px solid "+C.border,overflowY:"auto",zIndex:400,boxShadow:"4px 0 24px rgba(0,0,0,0.10)",animation:"slideInLeft 0.28s cubic-bezier(0.32,0,0.18,1)",padding:24}}>
                <ExportModal transactions={transactions} taxonomy={taxonomy} currency={currency} onClose={()=>setModal(null)} noOverlay={true}/>
              </div>
            );
            return (
              <div key={modalKey} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.30)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
                <ExportModal transactions={transactions} taxonomy={taxonomy} currency={currency} onClose={()=>setModal(null)}/>
              </div>
            );
          })()}
          {modal==="settings" && (function(){
            if(isWide) return (
              <div key={modalKey} style={{position:"absolute",top:0,left:210,bottom:0,width:380,background:C.surface,borderRight:"1px solid "+C.border,overflowY:"auto",zIndex:400,boxShadow:"4px 0 24px rgba(0,0,0,0.10)",animation:"slideInLeft 0.28s cubic-bezier(0.32,0,0.18,1)",padding:24}}>
            ...
            return (
              <div key={modalKey} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.30)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
                <SettingsModal taxonomy={taxonomy} vendorMap={vendorMap} rawTxs={rawTxs} currency={currency} onImport={handleSettingsImport} onClose={()=>setModal(null)}/>
              </div>
            );
          })()}
        </div>
        ...
        {/* Tabs — visible on all non-home tabs */}
