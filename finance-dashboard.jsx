...
// ─── Main App ─────────────────────────────────────────────────────────────────
// ─── Vendor View ─────────────────────────────────────────────────────────────
function VendorView({transactions, currency, dispRates, pFrom, pTo, selMonths, setRawTxs, vendorList}) {
  const today   = new Date();
  ...
  return (
    <div style={{padding:isWide?"24px 28px":"16px 12px",maxWidth:1200,margin:"0 auto"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:isWide?"2fr 1fr 1fr 1fr 1fr":"1fr",gap:12,marginBottom:14,alignItems:"stretch"}}>
        ...
        {/* HERO */}
        <div style={{..._surf,padding:isWide?"20px 24px":"20px 20px",display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
...
function PositionTab({financials, displayCurrency, displayRates, openModal, setManageInitSection, setManageInitFinancialSub}) {
...
function App() {
  ...
  var ADD_SECTIONS = [
    ...
    },
    {
      ...
      items:[
        {label:"Add transaction", action:function(){ setManualInitMode("transaction"); setManualFromHome(false); openModal("manual"); setAddSheetOpen(false); }},
      ]
    },
  ];
    ...
    return (
      <div style={{width:w,flexShrink:0,background:C.surface,borderRight:"1.5px solid "+C.border,
        ...
        <div style={{padding:sidebarCollapsed?"12px 0":"14px 16px 10px",borderBottom:"1px solid "+C.border,
          ...
          <div style={{width:44,height:44,borderRadius:8,overflow:"hidden",flexShrink:0,background:"transparent"}}>
            <img src="Home_financials_LOGO_White_Back.png" alt="Home Financials" style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}}/>
          </div>
          {!sidebarCollapsed&&(
            <div>
              <div style={{fontSize:12,fontWeight:700,color:C.text,whiteSpace:"nowrap"}}>Home Financials</div>
              <div style={{fontSize:9,color:C.dim,fontWeight:600,letterSpacing:"0.06em"}}>hf-v106</div>
            </div>
          )}
        </div>
        {/* Add / Import button */}
      ...
      <React.Fragment>
        <div style={{position:"fixed",top:0,left:0,right:0,height:52,
          ...
          <div style={{display:"flex",alignItems:"center",gap:7,padding:"0 10px",flexShrink:0,borderRight:"0.5px solid "+C.border}}>
            ...
            <div>
              <div style={{fontSize:11,fontWeight:700,color:C.text,whiteSpace:"nowrap",lineHeight:1.2}}>Home Financials</div>
              <div style={{fontSize:9,color:C.dim,fontWeight:600,letterSpacing:"0.06em"}}>hf-v106</div>
            </div>
          </div>
          {/* Nav items scrolling */}
            ...
            {tab!=="home"&&(
              <div>
                ...
                {tab==="position"&&(
                  <div style={card}>
                    {positionUnlocked
                      ? <PositionTab financials={financials} displayCurrency={displayCurrency} displayRates={displayRates} openModal={openModal} setManageInitSection={setManageInitSection} setManageInitFinancialSub={setManageInitFinancialSub}/>
                ...
                {tab==="vendors"&&<VendorView transactions={mTxs} currency={displayCurrency} dispRates={dispRates} pFrom={pFrom} pTo={pTo} selMonths={selMonths} setRawTxs={setRawTxs} vendorList={vendorList}/>}
                ...
                {tab==="transactions"&&(
                  <div style={card}>
                    ...
                    <div style={{display:"flex",gap:8,marginBottom:8,alignItems:"center",flexWrap:"wrap"}}>
                      ...
                      <button onClick={()=>setUncatOnly(v=>!v)} style={{padding:"5px 13px",borderRadius:20,border:`1px solid ${uncatOnly?C.accent:C.border}`,background:uncatOnly?C.accent:"transparent",color:uncatOnly?"#fff":C.muted,fontSize:12,fontWeight:uncatOnly?700:500,cursor:"pointer",fontFamily:"inherit"}}>Uncategorised</button>
                    </div>
                    <div style={{display:"flex",gap:8,marginBottom:8,alignItems:"center",flexWrap:"wrap"}}>
