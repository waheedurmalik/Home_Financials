...
function App() {
  ...
  var NAV_GROUPS = [
    { key:"input", icon:"📥", label:"Input data", items:[
      { icon:"📸", label:"PDF statement",      action:function(){ setManualInitMode("scan"); setManualFromHome(true); setModal("manual"); }},
      { icon:"🖼️", label:"Screenshot data",    action:function(){ setManualInitMode("scan"); setManualFromHome(true); setModal("manual"); }},
      { icon:"📊", label:"XLS spreadsheet",    action:function(){ setManualInitMode("scan"); setManualFromHome(true); setModal("manual"); }},
      { icon:"➕", label:"Add transaction",    action:function(){ setManualInitMode("transaction"); setManualFromHome(true); setModal("manual"); }},
      { icon:"💼", label:"Update position",    action:function(){ setManualInitMode("position"); setManualFromHome(true); setModal("manual"); }},
    ]},
    { key:"manage", icon:"🗂️", label:"Manage data", items:[
      { icon:"🔖", label:"Tag accounts",       action:function(){ setManageInitSection("transactions"); setManageInitFinancialSub(null); setModal("manage"); }},
      { icon:"🏷️", label:"Categories",         action:function(){ setManageInitSection("categories"); setManageInitFinancialSub(null); setModal("manage"); }},
      { icon:"🏪", label:"Vendors",            action:function(){ setManageInitSection("vendors"); setManageInitFinancialSub(null); setModal("manage"); }},
      { icon:"🏦", label:"Accounts",           indent:true, action:function(){ setManageInitSection("financial"); setManageInitFinancialSub("accounts"); setModal("manage"); }},
      { icon:"📈", label:"Investments",        indent:true, action:function(){ setManageInitSection("financial"); setManageInitFinancialSub("investments"); setModal("manage"); }},
      { icon:"🏠", label:"Properties",         indent:true, action:function(){ setManageInitSection("financial"); setManageInitFinancialSub("properties"); setModal("manage"); }},
      { icon:"💳", label:"Loans",              indent:true, action:function(){ setManageInitSection("financial"); setManageInitFinancialSub("loans"); setModal("manage"); }},
      { icon:"💰", label:"Set budgets",        action:function(){ setTab("budget"); }},
      { icon:"📊", label:"Forecast spend rows",action:function(){ setManageInitSection("forecast"); setManageInitFinancialSub(null); setModal("manage"); }},
    ]},
    { key:"tools", icon:"🔧", label:"Tools", items:[
      { icon:"💾", label:"Backup / restore",   action:function(){ setModal("settings"); }},
      { icon:"📤", label:"Export Excel",       action:function(){ setModal("export"); }},
      { icon:"🤖", label:"Gemini usage",       action:function(){ setManageInitSection("gemini"); setManageInitFinancialSub(null); setModal("manage"); }},
    ]},
  ];
    ...
    var panel = (
      <div style={panelStyle}>
        <div style={{padding:"6px 16px 10px",borderBottom:"1px solid "+C.border,marginBottom:8}}>
          <div style={{fontSize:9,color:C.dim,fontWeight:600,letterSpacing:"0.06em"}}>hf-v93</div>
        </div>
      ...
      {tab==="home" ? (
        ...
        <div style={{display:"flex",minHeight:"calc(100vh - 82px)",position:"relative"}}>
          ...
          {modal==="manual" && (function(){
            if(isWide) return (
              <div style={{position:"absolute",top:0,left:210,bottom:0,width:380,background:C.surface,borderRight:"1px solid "+C.border,overflowY:"auto",zIndex:400,boxShadow:"4px 0 24px rgba(0,0,0,0.10)",animation:"slideInLeft 0.28s cubic-bezier(0.32,0,0.18,1)"}}>
