...
function ManageModal({taxonomy,setTaxonomy,vendorMap,setVendorMap,vendorList,setVendorList,rawTxs,setRawTxs,financials,setFinancials,budgets,initialSection,singleSection,onClose}) {
...
// ─── Manual Entry Modal ───────────────────────────────────────────────────────
function ManualEntryModal({onImport, taxonomy, vendorMap, vendorList, financials, setFinancials, onClose, initialMode, singleSection}) {
...
function App() {
  ...
  var NAV_GROUPS = [
    { label:"Input data", items:[
      { icon:"📸", label:"PDF statement",      action:function(){ setManualInitMode("scan"); setManualFromHome(true); setModal("manual"); }},
      { icon:"🖼️", label:"Screenshot",         action:function(){ setManualInitMode("scan"); setManualFromHome(true); setModal("manual"); }},
    ...
    { label:"Manage data", items:[
      { icon:"📋", label:"All transactions",   action:function(){ setTab("transactions"); }},
      { icon:"🔖", label:"Tag accounts",       action:function(){ setManageInitSection("transactions"); setModal("manage"); }},
      { icon:"🏷️", label:"Categories",         action:function(){ setManageInitSection("categories"); setModal("manage"); }},
      { icon:"🏪", label:"Vendors",            action:function(){ setManageInitSection("vendors"); setModal("manage"); }},
      { icon:"🏦", label:"Financial position", action:function(){ setManageInitSection("financial"); setModal("manage"); }},
      { icon:"💰", label:"Set budgets",        action:function(){ setTab("budget"); }},
      { icon:"📈", label:"Forecast spend rows",action:function(){ setManageInitSection("forecast"); setModal("manage"); }},
    ]},
    { label:"Tools", items:[
      { icon:"📊", label:"Summary",            action:function(){ setTab("summary"); }},
      { icon:"🏪", label:"Vendor spend",       action:function(){ setTab("vendors"); }},
      { icon:"⚠️", label:"Spend alerts",       action:function(){ setTab("watchout"); }},
      { icon:"🔮", label:"3-year forecast",    action:function(){ setTab("forecast"); }},
      { icon:"🛡️", label:"Financial position", action:function(){ setTab("position"); }},
      { icon:"💾", label:"Backup / restore",   action:function(){ setModal("settings"); }},
      { icon:"📤", label:"Export Excel",       action:function(){ setModal("export"); }},
      { icon:"🤖", label:"Gemini usage",       action:function(){ setManageInitSection("gemini"); setModal("manage"); }},
    ]},
  ];
  ...
  function renderNav() {
    ...
    var panel = (
      <div style={panelStyle}>
        <div style={{padding:"10px 16px 14px",borderBottom:"1px solid "+C.border,marginBottom:8}}>
          <img src="Home_financials_LOGO_White_Back.png" alt="Home Financials" style={{width:"100%",maxWidth:160,height:"auto",display:"block"}}/>
          <div style={{fontSize:9,color:C.dim,marginTop:5,fontWeight:600,letterSpacing:"0.06em"}}>hf-v91</div>
        </div>
        {NAV_GROUPS.map(function(group,gi){
          return (
            <div key={gi} style={{marginBottom:4}}>
              <div style={{fontSize:9,fontWeight:800,color:C.dim,letterSpacing:"0.10em",textTransform:"uppercase",padding:"8px 16px 4px"}}>{group.label}</div>
