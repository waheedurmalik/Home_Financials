...
// ─── Manual Entry Modal ───────────────────────────────────────────────────────
function ManualEntryModal({onImport, taxonomy, vendorMap, vendorList, financials, setFinancials, onClose, initialMode, singleSection, noOverlay}) {
...
function App() {
  ...
  var NAV_GROUPS = [
    { key:"input", icon:"📥", label:"Input data", items:[
      { icon:"📸", label:"PDF statement",      action:function(){ setManualInitMode("scan"); setManualFromHome(true); openModal("manual"); }},
      { icon:"🖼️", label:"Screenshot data",    action:function(){ setManualInitMode("scan"); setManualFromHome(true); openModal("manual"); }},
      { icon:"📊", label:"XLS spreadsheet",    action:function(){ setManualInitMode("scan"); setManualFromHome(true); openModal("manual"); }},
      { icon:"➕", label:"Add transaction",    action:function(){ setManualInitMode("transaction"); setManualFromHome(true); openModal("manual"); }},
      { icon:"💼", label:"Update position",    action:function(){ setManualInitMode("position"); setManualFromHome(true); openModal("manual"); }},
    ]},
    { key:"manage", icon:"🗂️", label:"Manage data", items:[
      { icon:"🔖", label:"Tag accounts",       action:function(){ setManageInitSection("transactions"); setManageInitFinancialSub(null); openModal("manage"); }},
      { icon:"🏷️", label:"Categories",         action:function(){ setManageInitSection("categories"); setManageInitFinancialSub(null); openModal("manage"); }},
      { icon:"🏪", label:"Vendors",            action:function(){ setManageInitSection("vendors"); setManageInitFinancialSub(null); openModal("manage"); }},
      { icon:"🏦", label:"Accounts",           indent:true, action:function(){ setManageInitSection("financial"); setManageInitFinancialSub("accounts"); openModal("manage"); }},
      { icon:"📈", label:"Investments",        indent:true, action:function(){ setManageInitSection("financial"); setManageInitFinancialSub("investments"); openModal("manage"); }},
      { icon:"🏠", label:"Properties",         indent:true, action:function(){ setManageInitSection("financial"); setManageInitFinancialSub("properties"); openModal("manage"); }},
      { icon:"💳", label:"Loans",              indent:true, action:function(){ setManageInitSection("financial"); setManageInitFinancialSub("loans"); openModal("manage"); }},
      { icon:"📊", label:"Forecast spend rows",action:function(){ setManageInitSection("forecast"); setManageInitFinancialSub(null); openModal("manage"); }},
    ]},
    { key:"tools", icon:"🔧", label:"Tools", items:[
      { icon:"💾", label:"Backup / restore",   action:function(){ openModal("settings"); }},
      { icon:"📤", label:"Export Excel",       action:function(){ openModal("export"); }},
      { icon:"🤖", label:"Gemini usage",       action:function(){ setManageInitSection("gemini"); setManageInitFinancialSub(null); openModal("manage"); }},
    ]},
  ];
    ...
    var panel = (
      <div style={panelStyle}>
        <div style={{padding:"6px 16px 10px",borderBottom:"1px solid "+C.border,marginBottom:8}}>
          <div style={{fontSize:9,color:C.dim,fontWeight:600,letterSpacing:"0.06em"}}>hf-v94</div>
        </div>
              ...
              {!collapsed && group.items.map(function(item,ii){
                return (
                  <button key={ii} onClick={function(){ doAction(item.action); }}
                    style={{display:"flex",alignItems:"center",gap:9,width:"100%",padding:item.indent?"7px 16px 7px 34px":"9px 16px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"inherit",textAlign:"left",borderRadius:0}}>
                    <span style={{fontSize:item.indent?13:15,flexShrink:0,width:20,textAlign:"center",opacity:item.indent?0.8:1}}>{item.icon}</span>
                    <span style={{fontSize:item.indent?11.5:12.5,color:item.indent?C.muted:C.text,fontWeight:item.indent?400:500}}>{item.label}</span>
                  </button>
                );
              })}
            </div>
    ...
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"inherit"}}>
      ...
      {modal==="manual" && tab!=="home" && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.30)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <ManualEntryModal onImport={handleImport} taxonomy={taxonomy} vendorMap={vendorMap} vendorList={vendorList} financials={financials} setFinancials={setFinancials} onClose={()=>{setModal(null);setManualFromHome(false);}} initialMode={manualInitMode} singleSection={manualFromHome} noOverlay={true}/>
        </div>
      )}
          ...
          {modal==="manual" && (function(){
            if(isWide) return (
              <div key={modalKey} style={{position:"absolute",top:0,left:210,bottom:0,width:380,background:C.surface,borderRight:"1px solid "+C.border,overflowY:"auto",zIndex:400,boxShadow:"4px 0 24px rgba(0,0,0,0.10)",animation:"slideInLeft 0.28s cubic-bezier(0.32,0,0.18,1)"}}>
                <ManualEntryModal onImport={handleImport} taxonomy={taxonomy} vendorMap={vendorMap} vendorList={vendorList} financials={financials} setFinancials={setFinancials} onClose={()=>{setModal(null);setManualFromHome(false);}} initialMode={manualInitMode} singleSection={true} noOverlay={true}/>
              </div>
            );
            return (
