    ...
    if(editing.type==="investments") return (
      <div>
        ...
        <LabelRow label="Shares Now Available">
          <input type="text" inputMode="decimal" value={f.sharesAvailable!=null?f.sharesAvailable:""} onChange={e=>set("sharesAvailable",parseFloat(e.target.value)||0)} placeholder={"e.g. "+(f.initialShares||0)+" (leave blank to use total shares)"} style={inp2({fontFamily:"monospace"})}/>
        </LabelRow>
...
// ─── Main App ─────────────────────────────────────────────────────────────────
// ─── Vendor View ─────────────────────────────────────────────────────────────
function VendorView({transactions, currency, dispRates, pFrom, pTo, selMonths, setRawTxs, vendorList, setVendorMap, taxonomy}) {
  const today   = new Date();
      ...
      <div style={{...card,padding:18}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
            ...
            {selected.size>=1&&<button onClick={function(){ setCatEditOpen(function(o){return !o;}); setEditCat(Object.keys(taxonomy||{})[0]||""); setEditSub(Object.keys(Object.values(taxonomy||{})[0]?.subs||{})[0]||""); setRenameOpen(false); setNewName(""); }} style={btn(catEditOpen?C.accent:C.s2, catEditOpen?"#fff":C.muted, catEditOpen?"none":`1px solid ${C.border}`,11,"5px 10px")}>Edit category</button>}
          </div>
        </div>
        <input
          value={search}
...
function PositionTab({financials, setFinancials, displayCurrency, displayRates, openModal, setManageInitSection, setManageInitFinancialSub}) {
  ...
  investments.forEach(function(inv){
    ...
    var value = price * (inv.status !== "locked" ? availSh : (inv.shares || 0));
  ...
  function ItemRow({label, hint, value, isLiability, isSub, onEdit}) {
    ...
    var valueEl = (editBalances && onEdit)
      ? <input type="number" defaultValue={Math.round(Math.abs(value))}
                ...
                return (
                  <div key={inv.id} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"9px 0",borderBottom:"1px solid "+C.s2}}>
                    ...
                    {(editBalances&&!livePrice)
                      ? <input type="number" defaultValue={Math.round(toDisplay(value, inv.currency||"USD"))}
                          onBlur={function(e){
            ...
            return (
              <div key={prop.id} style={{padding:"9px 0",borderBottom:"1px solid "+C.s2}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  ...
                  {editBalances
                    ? <input type="number" defaultValue={Math.round(toDisplay(val, prop.currency||"AED"))}
                        onBlur={function(e){
                          var v=parseFloat(e.target.value);
                          if(!isNaN(v)) setFinancials(function(prev){
...
function FinancialPositionPanel({financials, setFinancials, onSave, onClose, displayCurrency}) {
  var dc = displayCurrency || "AED";
  ...
  function amtRow(itemId, label, existing, readOnly, onChangeCb, currencyHint) {
    ...
    return (
      <div style={{marginTop:8}}>
        ...
        {currencyHint&&editBalances&&!readOnly&&(
          <div style={{fontSize:10,color:C.dim,textAlign:"right",marginTop:2,paddingRight:2}}>
            {"Enter value in "+currencyHint+" — app converts to "+dc}
          </div>
        )}
      </div>
    );
    ...
    return investments.length===0 ? (
      ...
      return (
        <div key={inv.id} style={{background:C.s2,borderRadius:10,padding:"12px 14px",marginBottom:10,border:"1px solid transparent"}}>
          ...
          {amtRow(inv.id+"_shares", "Shares (set in Edit Data)", inv.shares?String(inv.shares):null, true, null)}
...
function ManualEntryModal({onImport, taxonomy, vendorMap, vendorList, financials, setFinancials, onClose, initialMode, initialScanMode, singleSection, noOverlay}) {
  ...
  var innerPanel = (
    <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:24,maxWidth:480,width:"100%",maxHeight:"94vh",overflowY:"auto"}}>
        ...
        {/* ── POSITION MODE ── */}
        {mode==="position"&&<FinancialPositionPanel financials={financials} setFinancials={setFinancials} onSave={onClose} onClose={onClose} displayCurrency={displayCurrency}/>}
      ...
      <div style={{width:w,flexShrink:0,background:C.surface,borderRight:"1.5px solid "+C.border,
        ...
        <div style={{padding:sidebarCollapsed?"12px 0":"14px 16px 10px",borderBottom:"1px solid "+C.border,
          ...
          <div style={{width:sidebarCollapsed?40:120,height:40,flexShrink:0,background:"transparent",display:"flex",flexDirection:"column",alignItems:sidebarCollapsed?"center":"flex-start"}}>
            ...
            {!sidebarCollapsed&&<div style={{fontSize:9,color:C.dim,fontWeight:600,letterSpacing:"0.06em",marginTop:2}}>hf-v118</div>}
          </div>
        </div>
        {/* Add / Import button */}
        ...
        {/* TOOLS nav — fixed just above footer, no flex growth */}
        ...
        <div style={{position:"fixed",top:0,left:0,right:0,height:52,
          ...
          <div style={{display:"flex",alignItems:"center",gap:7,padding:"0 10px",flexShrink:0,borderRight:"0.5px solid "+C.border}}>
            <div style={{width:90,height:30,flexShrink:0,background:"transparent",display:"flex",flexDirection:"column",alignItems:"flex-start"}}>
              ...
              <div style={{fontSize:9,color:C.dim,fontWeight:600,letterSpacing:"0.06em"}}>hf-v118</div>
            </div>
          </div>
          {/* Nav items scrolling */}
          ...
          <div onClick={function(){setDrawerOpen(false);}}
            ...
            <div onClick={function(e){e.stopPropagation();}}
              ...
              <div style={{marginTop:"auto",borderTop:"0.5px solid "+C.border,padding:"14px 16px",flexShrink:0}}>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div style={{fontSize:10,color:C.dim,fontWeight:600,letterSpacing:"0.06em"}}>hf-v118</div>
          ...
          <div style={{padding:isWide?"24px 28px":"16px 14px"}}>
            ...
            {tab!=="home"&&(
              <div>
                ...
                {tab==="vendors"&&<VendorView transactions={mTxs} currency={displayCurrency} dispRates={dispRates} pFrom={pFrom} pTo={pTo} selMonths={selMonths} setRawTxs={setRawTxs} vendorList={vendorList} setVendorMap={setVendorMap} taxonomy={taxonomy}/>}
