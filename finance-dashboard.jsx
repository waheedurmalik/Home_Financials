        ...
        {section==="vendors"&&(
          <div>
            ...
            <div style={{display:"flex",gap:2,background:C.s2,borderRadius:10,padding:3,marginBottom:12}}>
              {[["all","All Vendors"],["unassigned","⚠️ No Category Yet"]].map(([v,l])=>(
                <button key={v} onClick={()=>{setVendorTab(v);setVendorSel(new Set());}} style={{flex:1,padding:"7px 0",borderRadius:8,border:"none",background:vendorTab===v?C.surface:"transparent",color:vendorTab===v?C.text:C.muted,fontWeight:vendorTab===v?700:500,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>{l}</button>
              ))}
            </div>
            <input
            ...
            />
            {/* Vendor list */}
            <div style={{maxHeight:220,overflowY:"auto",border:`1px solid ${C.border}`,borderRadius:10,background:C.bg,marginBottom:14}}>
              {(()=>{
                const allV=[...new Set((rawTxs||[]).map(t=>t.description))].filter(Boolean).sort();
                const filtered=allV
                  .filter(v=>vendorTab==="unassigned"?!vendorMap[v.toLowerCase().trim()]:true)
                ...
                return filtered.map(v=>{
                  const on=vendorSel.has(v);
                  const vm=vendorMap[v.toLowerCase().trim()];
                  return (
                    <div key={v} style={{borderBottom:`1px solid ${C.s2}`,background:on?"rgba(42,157,111,0.06)":"transparent"}}>
                      {renamingVendor&&renamingVendor.old===v ? (
                        <div style={{padding:"8px 14px",display:"flex",gap:8,alignItems:"center"}} onClick={e=>e.stopPropagation()}>
                          ...
                          <button onClick={()=>{
                            ...
                            setRawTxs(prev=>prev.map(t=>t.description===oldName?{...t,description:newName}:t));
                            // Update vendorMap key
                            const oldKey=oldName.toLowerCase().trim();
                            const newKey=newName.toLowerCase().trim();
                            const newMap={...vendorMap};
                            if(newMap[oldKey]){newMap[newKey]=newMap[oldKey];delete newMap[oldKey];}
                            setVendorMap(newMap);
                        ...
                        <div onClick={()=>setVendorSel(prev=>{const s=new Set(prev);s.has(v)?s.delete(v):s.add(v);return s;})} style={{padding:"9px 14px",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
                          ...
                          <span style={{flex:1,fontSize:13,fontWeight:500,color:C.text}}>{v}</span>
                          {vm&&<span style={{fontSize:11,color:C.muted}}>{vm.category} › {vm.subcategory}</span>}
                          <button onClick={e=>{e.stopPropagation();setRenamingVendor({old:v,newName:v});}} style={{...btn(C.s2,C.muted,`1px solid ${C.border}`,10,"2px 7px"),flexShrink:0}}>✏️</button>
                        </div>
                      )}
                    </div>
                  );
            ...
            {vendorSel.size>0&&(
              <div style={{background:C.s2,borderRadius:12,padding:"14px 16px",marginBottom:14}}>
                <div style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:10}}>Assign to {vendorSel.size} vendor{vendorSel.size!==1?"s":""}</div>
                ...
                <button
                  onClick={()=>{
                    ...
                    const newMap={...vendorMap};
                    vendorSel.forEach(v=>{newMap[v.toLowerCase().trim()]={category:vendorCat,subcategory:vendorSub,txType:vendorTxType||"standard"};});
                    setVendorMap(newMap);
                    if(setRawTxs) {
                      setRawTxs(prev=>prev.map(t=>{
                        if(vendorSel.has(t.description)) return {...t,category:vendorCat,subcategory:vendorSub,txType:vendorTxType||"standard",_manual:true};
                        return t;
                      }));
                    }
                    setVendorSaved(true);
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(13,15,14,0.96)",zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:28,maxWidth:420,width:"100%"}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:4}}>
          <button onClick={()=>onSave(cat,sub,txType,scope,accountId||null)} style={{...btn(C.accent,"#fff","none",14,"12px 0"),flex:1}}>Save</button>
...
function VendorView({transactions, currency, dispRates, pFrom, pTo, selMonths}) {
  ...
  const [vendorSort, setVendorSort] = useState("name"); // name | amount | date

  // Build vendor last-transaction-date map
  const vendorLastDate = useMemo(()=>{
    const m={};
    transactions.forEach(t=>{ if(!m[t.description]||t.date>m[t.description]) m[t.description]=t.date; });
    return m;
  },[transactions]);
  ...
  const allVendors = useMemo(()=>{
    const vs=[...new Set(transactions.map(t=>t.description))];
    if(vendorSort==="name")   vs.sort((a,b)=>a.localeCompare(b));
    if(vendorSort==="amount") vs.sort((a,b)=>(vendorTotal[b]||0)-(vendorTotal[a]||0));
    if(vendorSort==="date")   vs.sort((a,b)=>(vendorLastDate[b]||"").localeCompare(vendorLastDate[a]||""));
    return vs;
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{...card,padding:18}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
            <Pill options={[["name","A–Z"],["amount","Value ↓"],["date","Recent ↓"]]} value={vendorSort} onChange={setVendorSort}/>
      ...
      {selected.size>0&&(()=>{
        const selTxs=transactions
          .filter(t=>selected.has(t.description))
                ...
                : selTxs.map((tx,i)=>{
                  ...
                  return (
                    <div key={i} style={{padding:"10px 14px",display:"flex",alignItems:"center",gap:10,borderBottom:`1px solid ${C.s2}`}}>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{tx.description}</div>
                        <div style={{fontSize:11,color:C.muted,marginTop:2}}>{fmtDNum(tx.date)} · {tx.category} › {tx.subcategory}</div>
                      </div>
                      <TypeBadge txType={tx.txType} small/>
...
function App() {
  ...
  function handleRemap(cat,sub,txType,scope,newAccountId) {
    ...
    if(scope==="vendor") {
      ...
      setRawTxs(prev=>prev.map(t=>{
        ...
        if(isSame) return {...t,category:cat,subcategory:sub,txType,_manual:false};
        return t;
      }));
    } else {
                      ...
                      <div style={{display:"flex",gap:5,flexShrink:0}} onClick={e=>e.stopPropagation()}>
                        {isEditing?(
                          ...
                          <>
                            ...
                            <button onClick={e=>{e.stopPropagation();if(window.confirm("Delete?"))deleteTransactions(function(t){return t.date===tx.date&&t.description===tx.description&&t.amount===tx.amount;});}} style={btn("rgba(245,118,118,0.1)",C.danger,`1px solid ${C.danger}44`,11,"3px 7px")}>🗑</button>
                          </>
                        )}
                      </div>
                    </div>
