    ...
    <div style={{position:"fixed",inset:0,background:"rgba(13,15,14,0.96)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{position:"relative",background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:28,maxWidth:560,width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        {section==="vendors"&&(()=>{
          ...
          const allVendors=(vendorList||[]).slice().sort((a,b)=>a.name.localeCompare(b.name));
  ...
  async function runScan() {
    ...
    try {
      ...
      if(scanMode==="pdf") {
        ...
        // ── Step 2: Gemini vendor suggestions for unknown descriptions only ──
        var knownDescs=new Set(Object.keys(vendorMap||{}));
        var uniqueUnknown=[...new Set(parsedTxs.map(function(t){return t.description;}))].filter(function(d){return !knownDescs.has(d);});
        // suggestions keyed by original description string (index-matched as fallback)
        var suggestionsByDesc={};
        var suggestionsByIdx={}; // index-based fallback in case Gemini cleans description field
        if(uniqueUnknown.length>0) {
          addLog("Asking Gemini for vendor suggestions on "+uniqueUnknown.length+" new descriptions…");
          try {
            var {auth}=getFB(); var user=auth.currentUser; var token=await user.getIdToken();
            var suggestRes=await fetch(CLOUD_FN_URL,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+token},body:JSON.stringify({descriptionList:uniqueUnknown})});
            if(suggestRes.ok){
              var suggestData=await suggestRes.json();
              addLog("⚙ Suggest raw: "+(JSON.stringify(suggestData)||"").slice(0,300));
              var arr=suggestData.suggestions||[];
              arr.forEach(function(s,i){
                if(s.description) suggestionsByDesc[s.description]=s;
                suggestionsByIdx[i]=s; // always store by index
              });
              // count how many uniqueUnknown we can now satisfy (by desc or by index)
              var matched=uniqueUnknown.filter(function(d,i){return suggestionsByDesc[d]||suggestionsByIdx[i];}).length;
              addLog("✓ Vendor suggestions received for "+matched+" descriptions");
            } else {
              addLog("⚠ Vendor suggestions HTTP "+suggestRes.status+" — continuing without");
            }
          } catch(e){ addLog("⚠ Vendor suggestions unavailable — "+e.message); }
        }

        // ── Step 3: Apply vendorMap + suggestions ──
        // Build a map from description → suggestion using index-based matching
        var suggestions={};
        uniqueUnknown.forEach(function(d,i){
          suggestions[d]=suggestionsByDesc[d]||suggestionsByIdx[i]||null;
        });
        txs = parsedTxs.map(function(t) {
          var mapped=(vendorMap||{})[t.description];
          var suggested=suggestions[t.description];
          return Object.assign({},t,{
            vendor:mapped?(mapped.vendor||null):(suggested&&suggested.vendor?suggested.vendor:null),
            category:mapped?mapped.category:(suggested?suggested.category||"":""),
            subcategory:mapped?mapped.subcategory:(suggested?suggested.subcategory||"":"")
          });
        });
