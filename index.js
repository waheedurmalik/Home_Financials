...
functions.http("parseStatement", async function(req, res) {
  ...
  try {
    ...
    // ── VOICE ENRICHMENT MODE ──
    if (body.mode === "voice") {
      const transcript   = (body.transcript   || "").slice(0, 500);
      const vendorNames  = (body.vendorNames  || []).slice(0, 200);
      const categoryNames= (body.categoryNames|| []).slice(0, 50);
      ...
      const prompt =
        ...
        "The user spoke this to log a transaction:\n\"" + transcript + "\"\n\n" +
        "Known vendor names (match against these if the transcript is close, including spelled-out letters like C-A-R-R-E-F-O-U-R):\n" +
        vendorNames.join(", ") + "\n\n" +
        "Known categories:\n" + categoryNames.join(", ") + "\n\n" +
        "RULES:\n" +
        "- If you see individual capital letters separated by spaces or hyphens (e.g. C-A-R-R-E-F-O-U-R or C A R R E F O U R), reconstruct the intended word and match to the vendor list.\n" +
        "- Resolve relative dates: 'yesterday', 'last Monday', '5th Jan', '5th of January 2025' → YYYY-MM-DD. Default to today if no date mentioned.\n" +
        "- amount: extract the numeric value only. 'two hundred and thirty' = 230, 'three fifty' = 350.\n" +
        "- isCredit: true for salary/income/refund, false for expense/spend/paid.\n" +
        "- vendor: match to known vendor list if possible. Otherwise extract the merchant name from transcript.\n" +
        "- category/subcategory: pick from known categories if confident. Empty string if not confident.\n" +
        "- confidence: for each field, 'high' if certain, 'low' if guessed.\n\n" +
        "Return ONLY valid JSON, no markdown, no explanation:\n" +
        "{\"date\":\"YYYY-MM-DD\",\"description\":\"original transcript cleaned up\",\"amount\":number_or_null,\"isCredit\":boolean,\"vendor\":\"string\",\"category\":\"string\",\"subcategory\":\"string\",\"confidence\":{\"date\":\"high|low\",\"amount\":\"high|low\",\"vendor\":\"high|low\",\"category\":\"high|low\"}}";
      ...
      catch(e) { res.status(500).json({ error: "Gemini parse failed", raw }); return; }
    ...
    if (body.images && body.images.length > 0) {
      ...
      streamS({ log: `Reading ${images.length} screenshot${images.length===1?"":"s"} with Gemini (parallel)…` });
      ...
      const PROMPT =
        ...
        "- HSBC current account: 'Deposits' column = isCredit:true, 'Withdrawals' column = isCredit:false\n" +
        "- Strip prefixes: IAP-, NFC-, AP-PAY-, card numbers, city/country suffixes like 'Dubai AE'\n" +
        "- Output COMPLETE JSON only. Never truncate. Output every single transaction row.";

      // Mutex queue — ensures res.write() calls never overlap even when pages finish simultaneously
      let writeChain = Promise.resolve();
      function safeStream(obj) {
        writeChain = writeChain.then(function() { res.write(JSON.stringify(obj) + "\n"); });
        return writeChain;
      }

      // Keepalive: ping every 15s so Safari doesn't drop the stream during parallel processing
      const keepalive = setInterval(function() { safeStream({ ping: 1 }); }, 15000);

      const results = await Promise.all(images.map(async function(img, i) {
        try {
          const parts = [
          ...
          const response = await ai.models.generateContent({
            ...
            config: { maxOutputTokens: 16384, temperature: 0.1, responseMimeType: "application/json" }
          });
          const raw = response.text || "";
          const txs = parseGeminiResponse(raw, i);
          safeStream({ log: `Page ${i+1}: extracted ${txs.length} transaction${txs.length===1?"":"s"}` });
          return txs;
        } catch (e) {
          console.log(`Screenshot page ${i+1} error:`, e.message);
          safeStream({ log: `Page ${i+1}: error — ${e.message}` });
          return [];
        }
      }));

      clearInterval(keepalive);
      // Wait for all queued writes to flush before sending done
      await writeChain;
      const allScreenshotTxs = results.flat();
      const screenshotFinal = deduplicate(allScreenshotTxs);
      // Route done signal through safeStream so it queues after any final writes
      safeStream({ done: true, transactions: screenshotFinal, piiReport: { count: 0, types: {}, samples: [] }, chunks: images.length });
      // Wait for done write to flush, then end the response
      await writeChain;
      res.end();
      return;
    }
